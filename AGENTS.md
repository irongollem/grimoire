...
**Model name only — no parenthetical session details.** `Codex Opus 5`, not `Codex Opus 5 (1M context)`. Git keys a co-author on the whole name, so the suffix forks one model into two contributors: across the history so far `Codex Opus 5` splits 123/19, `Codex Opus 4.8` 203/18. A context window is a property of the session that produced the commit, not of the change or of who wrote it, and nobody reading the log later can act on it.
