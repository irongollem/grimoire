...
- **The standing security-advisor baseline is 102 findings, last measured against production 7 Sep 2026, immediately after epic #780 deployed (was 87 on 12 Aug, 95 on 23 and 28 Aug); the audit behind it was 9 Aug 2026 — not merely inherited.** The 28 Aug measurement actually returned **96**; it is 95 again because that reading found a real leak, fixed in `20260828202800` — see the `security_definer_view` bullet below before trusting the number. Treat that number as the line: a new finding is a regression, but the 102 are not a backlog to "clean up". Note the breakdown below sums to the total — an earlier revision of this line said 92 over a breakdown summing to 90, which is how a baseline stops being checkable. 102 = 87 + 5 + 9 + 1. They are, in full:
