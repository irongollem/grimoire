...
- **The standing security-advisor baseline is 96 findings, last measured against production 28 Aug 2026 at 95 and raised by one deliberate addition since (was 87 on 12 Aug, 95 on 23 Aug); the audit behind it was 9 Aug 2026 — not merely inherited.** The 28 Aug measurement actually returned **96**; it is 95 again because that reading found a real leak, fixed in `20260828202800` — see the `security_definer_view` bullet below before trusting the number. The 96th is `session_proposal_invites`, added deliberately by `20260831082156` and accounted for below. Treat the number as the line: an *unaccounted* finding is a regression, but the 96 are not a backlog to "clean up" — and when you add one on purpose, raise the number here in the same change, or the next reader compares against a line that has quietly stopped being true. Note the breakdown below sums to the total — an earlier revision of this line said 92 over a breakdown summing to 90, which is how a baseline stops being checkable. They are, in full:
  ...
  - **10 `rls_enabled_no_policy` (INFO)** — the eight `*_embeddings` tables, `disposable_email_domains`, and `session_proposal_invites`. RLS on with *no* policy is deny-all to `anon`/`authenticated`; these are read only through `SECURITY DEFINER` RPCs and written only by edge functions, so the absence of policies is the lockdown, not an oversight. Adding policies here would *widen* access. `session_proposal_invites` (`20260831082156`) is the sharpest case: its rows are RSVP capability tokens, so a `select` policy scoped to the campaign would hand every DM the means to accept on each of their players' behalf. Its three RPCs are `service_role`-only, like `withdraw_waitlist_consent`, so they add nothing to the definer count above — that count is of functions **PostgREST can reach**, and these are not among them.
