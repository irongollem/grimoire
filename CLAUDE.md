...
**State of play, 8 Sep 2026.** A designed-vs-shipped audit against all fourteen boards found the schema right and every live board's *composition* missing — the runner has no place column and no story spine, the graph draws prep chips instead of ledger deltas, the beat panel is a generic condition→action form, prep showed no per-beat state. The last two and the overview-modal navigation were fixed as design-independent bugs; the shared vocabulary is `src/lib/quests/ledger.ts`. **The runner, graph, beat-panel, site and player rebuilds are deliberately not started**: the maintainer is reworking the canvas, and building those against the current boards was declined. Re-read the canvas when the rework lands, re-diff, then build — with the `frontend-design` skill's rules in every executor spec.
