...
**There are three designs, and they say different things.**

1. **The model** is the 14-board canvas — <https://claude.ai/code/artifact/30dd83c5-1688-4848-b168-86e9af9ae34e> — boards `Main` `Today` `Site` `SiteEditor` `Graph` `Runner` `Prep` `Player` `Plan` `Cutover` `Gaps` `DirectionA/B/C`. Its `Main` board still holds: beats are events, objectives are state, a beat raises / achieves / fails / reveals; the dungeon is a place in the Atlas. Its *surface* boards were superseded by the second design. (Content lives in the `appifact-doc` state block, not the DOM: after `Artifact action:"read"`, `json.loads(re.search(r'<script[^>]*id="appifact-doc"[^>]*>(.*?)</script>', html, re.S).group(1))["content"]["files"]`.)
2. **The surfaces** are **`quests/Quest Manager Redesign.html`** in the Grimoire project on Claude Design (with `qm-redesign.css`, `qm-icons.js`), read through the design-sync tool (`DesignSync get_file`, project `9f4667da-cdb6-42c7-87c7-b571f4f51d6a`). Eight frames: `01 Delta` `02 Story flow` `03 Inspector` `04 Run` `05 Advance` `06 Site` `07 Log` `08 Player`. Its first frame names what the model could not say — a quest holding several cursors (threads), a route that says "and also" (parallel), payoffs in one list — and the rest draw every DM and player surface around that. Epic #850 implemented it in full on 8 Sep 2026; its body is the plan and the decisions.

3. **The site** is **`atlas/Sites & Cartographer.html`** in the same project (with `sites.css`, `sa-plan.js`, `sa-icons.js`), sixteen frames: `01 Delta` `02 Atlas place pane` `03 Floor plan` `04 Cartographer` `05 Publish review` `06 Levels` `07 Zones` `08 Run` `09 Data model` `10 Dungeon Craft` `11 Craft both ways` `12 Tracing` `13 Battle map` `14 Room audio` `15 Quest and site` `16 Fog and player view`. Its first frame says what changed in one line: the Cartographer threw its structure away at bake time, so the DM redrew by hand in the Atlas what the editor already knew. Epic #868 implemented it in full on 9 Sep 2026 in four waves; its body holds the decisions the sheet left to the implementation (door play state as `door_id` facts in the state log, `dungeon_maps.rev`, a `zone` layer inside `layers` jsonb, an arch cutting the fill like a door). One frame-15 line — "Cleared can satisfy an objective" — was split out as #869 because the consequence engine keys every firing on a beat transition, and closed the same day the way `assert_quest_objective_status` already worked: an `assert` transition per (state event, quest). The maintainer's framing while it was built: "a room or other subpart from a site is a zoomed in beat", which is why the run surface reuses the beat's read-aloud and payoff pieces rather than inventing room equivalents.

4. **The site workbench** is epic #884 (14 Sep 2026), a decision record rather than a sheet, and it overrides frames 02/03 of the third design where they disagree. Three decisions: a site's map is a **stack of three layers** — Plan (regions, doors, placements), Drawing (`dungeon_maps` via `source_map_id`, displayed as a transparent bake in `map_layer_url`) and Picture (`map_url`: a scan, or the AI styler's render of the Drawing, which *bubbles down* to this layer) — each independently empty, bound by one grid; a site has **Browse and Build** states (`?build=true`), Run being a layout that gates nothing of its own, and the structural panels' add rows, handles and Trace/Bind show in Build only (this reverses #783's "always-editable"); and the **Cartographer editor runs in place** inside Build's map area as well as at `/cartographer` (this reverses #807's "no live canvas in the Atlas" for editing, not for display). Publish writes `map_layer_url`, never `map_url`. Read the epic body before touching any of it.

**How this went wrong once, so it does not repeat.** Epic #780 shipped the model and reviewed it thoroughly — every pass compared the code **against itself**, none against the canvas — so the primitives landed and none of the composition did. Then a session invented a parallel redesign from screenshots. Do not theorise about a decision that is already drawn; open the frame.

**What "read the design" means in practice.** Every executor spec for quest UI names its frame and quotes its captions; the overseer looks at each surface in the running app against that frame before calling it done. The `frontend-design` skill's rules are part of every such spec (the maintainer's words: "make sure it stays good looking this time").
...
Two things go wrong with versions, and CI now catches both (`node scripts/migration-rebase/cli.mjs --check`, run in the `database` job):
...
- **The standing security-advisor baseline is 109 findings — 106 measured against production 8 Sep 2026 immediately after epic #850 deployed, plus one added deliberately on 16 Sep 2026 (see the prompt-screening note below), minus two removed on 23 Sep 2026 with the Pro waitlist (see its note below), plus five added deliberately on 25 Sep 2026 with the demo campaign (see its note below; 109 measured against production that morning, before the fifth), minus one removed on 26 Sep 2026 with the companion party-notes RPC (see its note below) (was 102 on 7 Sep after #780, 87 on 12 Aug, 95 on 23 and 28 Aug); the audit behind it was 9 Aug 2026 — not merely inherited.** The 28 Aug measurement actually returned **96**; it is 95 again because that reading found a real leak, fixed in `20260828202800` — see the `security_definer_view` bullet below before trusting the number. The 103rd is `session_proposal_invites`, added deliberately by `20260908084110` and accounted for below. Treat the number as the line: an *unaccounted* finding is a regression, but the 103 are not a backlog to "clean up" — and when you add one on purpose, raise the number here in the same change. That is not hypothetical bookkeeping: this entry was raised to 96 on 31 Aug for exactly that table, and a later merge took this paragraph wholesale from `main` and dropped the accounting with it, which is how a baseline quietly stops being true even when someone did the right thing the first time. Note the breakdown below sums to the total — an earlier revision of this line said 92 over a breakdown summing to 90, which is how a baseline stops being checkable. 109 = 93 + 5 + 10 + 1. They are, in full:
  - **98 `*_security_definer_function_executable` (93 authenticated + 5 anon).** The advisor flags *every* `SECURITY DEFINER` function PostgREST can reach. In an app whose entire write path is RPCs, that is the expected shape, not a defect — the question that matters is whether each one authorizes internally, and every one does, via `auth.uid()`, a `private.*` helper, or the `auth.jwt() -> 'app_metadata' ->> 'role'` admin check. Five are deliberately anon-reachable: `validate_app_invite` (runs before login by definition — the token is the credential) and the four `get_library_*_sources` (shared content, intentionally not account-gated). That set of five is pinned by `supabase/tests/anon_rpc_surface.test.sql`, so a sixth cannot arrive by accident — which is how the three in #650 arrived, via the `PUBLIC` default rather than any explicit grant. If you add one on purpose, add it to the test and say why there. Three of them arrived with #642 (`20260809214703`): `admin_set_user_plan`, `admin_set_user_suspended` and `admin_grant_credits`, each gated on `private.is_app_admin()`. They exist *because* of a security requirement rather than in spite of one — they replaced direct client table writes, so the audit entry cannot be skipped — which is the shape to expect whenever a privileged action stops being a PostgREST call. **Where the extra ten since the audit came from:** the quest-runtime RPCs of `20260810*` (`get_quest_runtime_context`, `transition_quest_runtime`, `improvise_quest_runtime`, `convert_quest_to_flow` and their siblings), the two DSR writers of #643, and `admin_remove_waitlist_email` from #638 (since removed — see below). Three more landed with the campaign session (#758): `start_campaign_session` and `end_campaign_session` in `20260822234841`, both gated on `coalesce(private.is_campaign_dm(...), false)`, and `get_player_session_state` in `20260823212755` — the players' projection, gated on `private.is_campaign_member` and returning strictly less than the row. All three revoked from `anon`. The five before them came from the per-quest runtime cursor (#755) and quest status promotion (#756). Three more landed with `20260814221409` (durable characters): `attach_party_member_to_campaign`, `detach_party_member_from_campaign` and `clone_party_member`, each re-deriving identity from `auth.uid()` first — verified during the 19 Aug 2026 document-items audit, which itself added zero (its helper lives in `private`, its trigger functions are revoked). Each is a write path that moved from client table access into a gated RPC, which is the direction that grows this count on purpose. Note the shape of the number: it rises with ordinary feature work, so a **rising** count is not by itself the regression signal — a name you cannot account for is. **Seven more landed with epic #780 on 7 Sep 2026**, taking authenticated from 80 to 87: `assert_quest_runtime` (#796), `get_player_visible_site_state` (#798), `dispatch_loot` and `get_loot_placements` (#830 — renamed from `dispatch_quest_beat_loot` / `get_quest_beat_loot` when rooms gained the same verb), `resolve_monster_references` (#837), `resolve_item_references` (#838) and `get_unembedded_content_counts` (#841). All seven verified against production the day they shipped: `SECURITY DEFINER` with `search_path = public, private`, authorizing internally, and **none executable by `anon`**. Three negate their predicate and therefore `coalesce(..., false)`; the other four use it affirmatively in a `WHERE`, where NULL excludes the row and so denies — the asymmetry item 3 above turns on. `anon` did not move, which is the number that would have mattered. **Three more landed with epic #850 on 8 Sep 2026, taking authenticated from 87 to 90 — measured against production the same evening, composition and total exactly as predicted:** `open_quest_thread` and `close_quest_thread`, both gated on `private.is_campaign_dm` with `coalesce(..., false)`, revoked from `anon`. A third, `ensure_quest_main_thread` (`20260908210325`), is the same shape — DM-gated, revoked from `anon`. `anon` still 5 — the number that would have mattered did not move. **One more on 16 Sep 2026, taking authenticated from 90 to 91:** `get_prompt_screening_hints` (`20260915225357`), the admin read over the image-prompt screening log — `security definer`, `search_path = public, private`, `coalesce(private.is_app_admin(), false)` as its first act, revoked from `public, anon`. Verified structurally against a local apply before push (definer set, search_path pinned, `anon` cannot execute). **Production re-check done 18 Sep 2026, after the migration landed: total 107, composition 91 + 5 + 10 + 1 — exactly as predicted, `anon` unmoved at 5.** The function's body was read back from production the same day: `coalesce(private.is_app_admin(), false)` is its first act and it raises before touching a row. Its table, `prompt_screenings`, carries two select policies (own row + admin), so it does **not** add to the `rls_enabled_no_policy` count below — that stays 10. **Two fewer on 23 Sep 2026, when the Pro waitlist was removed (`20260923181129`) — predicted, re-measure against production after it deploys:** dropping `admin_remove_waitlist_email` takes authenticated from 91 to 90, and dropping `disposable_email_domains` takes the no-policy count below from 10 to 9. `withdraw_waitlist_consent` went too, but it was `service_role`-only and never counted. `anon` does not move. **Three more on 25 Sep 2026 with the demo campaign (#912, `20260925002215`) — predicted, re-measure after it deploys:** `load_demo_campaign` and `get_demo_status` (each raises on a null `auth.uid()` first, and neither takes an id from the caller: the template and the copy's owner are server-derived) and `publish_demo_version` (`coalesce(private.is_app_admin(), false)` as its first act). All three revoked from `public, anon`, taking authenticated from 90 to 93 — **measured against production 25 Sep 2026 after deploy: exactly those three, `anon` unmoved.** A fourth, `set_demo_offered` (`20260925054923`, admin-only via `coalesce(private.is_app_admin(), false)`, acts on the one template rather than a caller-supplied id), takes it to 94. Their helpers — `private.copy_demo_template`, `private.purge_demo_campaign`, `private.remap_demo_ids` — live in `private` and are revoked from every client role. **The prediction for this migration was wrong in one place, and the miss is the lesson:** it said the config table `private.demo_campaign_tables` would not move the no-policy count because it is outside `public`. It did move it: the advisor's `rls_enabled_no_policy` lint reads `private` too, unlike the definer lint. See the next bullet. `anon` does not move. **One fewer on 26 Sep 2026, taking authenticated from 94 to 93. Predicted; re-measure after it deploys:** `update_companion_party_notes` was dropped by `20260926155338`. Nothing had called it since companion notes moved to `entity_notes` (#587), and it negated `private.is_campaign_member` without a `coalesce`, so a companion with a null `campaign_id` fell straight through item 3's NULL bypass. `anon` does not move.
  - **10 `rls_enabled_no_policy` (INFO)** — the eight `*_embeddings` tables, `session_proposal_invites`, and `private.demo_campaign_tables` (the demo copy's table classification, #912, read only by the demo definer functions and revoked from every client role — added 25 Sep 2026; `disposable_email_domains` had been the tenth until the waitlist removal of 23 Sep 2026). RLS on with *no* policy is deny-all to `anon`/`authenticated`; these are read only through `SECURITY DEFINER` RPCs and written only by edge functions, so the absence of policies is the lockdown, not an oversight. Adding policies here would *widen* access. `session_proposal_invites` (`20260908084110`) is the sharpest case: its rows are RSVP capability tokens, so a `select` policy scoped to the campaign would hand every DM the means to accept on each of their players' behalf, and a self policy would hand a player a token they already received by mail. Its three RPCs are `service_role`-only, like `withdraw_waitlist_consent`, so they add nothing to the definer count above — that count is of functions **PostgREST can reach**, and these are not among them.
...
- **"Copy to campaign…" does not navigate after it succeeds, and that is not a Post-Mutation Navigation violation.** The rule sends a create back to the list because the list *is* the confirmation. A copy-to-campaign (#598) creates its row in **another campaign**, which neither the page the DM is standing on nor its list can show — navigating to either would confirm nothing and would abandon the record they are still working on. The toast naming the destination ("Copied \"Ashen Warden\" to Curse of Strahd.") is the only confirmation available, which is why `CopyToCampaignDialog` emits the target's name alongside the count rather than just the count. The same-scope duplicate actions beside it — `ItemDetail`'s `cloneItem()`, `MonsterDetail`'s `duplicate()`, `ClassList`'s `duplicate()` — do land in this scope and keep their navigation exactly as the rule requires. The test is where the new row lands, not that a row was created.
