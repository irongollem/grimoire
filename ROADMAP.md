...
- [x] **AI: Split text/image API keys + Gemini Flash text provider** (irongollem/grimoire#112, irongollem/grimoire#115) — campaigns now store a separate `text_api_key` for text generation (NPC/monster/item/spell/puzzle) alongside the existing `openai_api_key` for image generation; `getTextProvider()` falls back to the image key if no text key is set; provider is auto-detected from key prefix — `AIza…` routes to Gemini Flash (`gemini-2.0-flash`) with JSON mode via `responseMimeType`, `sk-…` routes to OpenAI; `AiTab.vue` split into dedicated Text AI and Image AI key sections, each with masked input and provider links
