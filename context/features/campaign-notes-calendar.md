...
It exists because CSS grid places row by row and a row is as tall as its tallest item, so _Next session_ — one line of text — used to sit in a row beside an eight-row _Quests_ and waste everything under it, while the fourth cell wrapped to a new row instead of filling that hole.
...
**Persistence (#762).** A layout the DM has rearranged lives in `dashboard_layouts` (migration `20260823214517`), keyed `(user_id, campaign_id, surface)` with the arrangement in a `layout` jsonb column. `DashboardView.vue` reads it through `useDashboardLayout(view)` (`src/composables/dashboard/useDashboardLayout.ts`), which returns the _merged_ layout — never a loading state, because an absent or still-loading row merges to `DEFAULT_LAYOUTS[surface]`, so a DM who never customized sees exactly the dashboard above with no flash of empty grid.
...
2. **`configureEntry` replaces the whole blob, never merges.** An editor always emits the complete shape it owns. A merge would make a setting impossible to _unset_, which is the bug a settings dialog grows on its second field.
...
`isDefaultLayout` compares settings as _configuration_ rather than as objects — absent, `{}` and `{ k: undefined }` all agree, and key order does not matter, so a layout never differs from its own jsonb round trip. No default layout ships a configurable widget today, so that comparison cannot fire; it is written now so that the first one that does not silently stop Reset from offering itself.

**Removing a configured widget discards its settings.** The entry goes, and `settings` goes with it; re-adding from the shelf starts fresh. The announcement says "removed to the shelf" because nothing is _deleted_ from the campaign — but the arrangement of that instance is not kept anywhere.
...
**Roll a table (`RollTableWidget.vue`)** — one saved roll table (#120's entity), rolled in place. Configurable, `maxInstances: 4`, picker-only. The entity had existed for months reachable only from Dungeon Craft, which is where you _build_ a wandering-monster table; this is where you use one.

`src/lib/dashboard/rollTableCard.ts` resolves in **three** states, not two, and that is the difference from the DM screen card: a roll table is a campaign row, so its id can be valid at save time and gone by the next read (deleted, or a campaign switch). `none` = the campaign has no tables; `ready`; `missing` = the layout names one this campaign does not have. A `missing` card **says so and rolls nothing** — substituting the next table would have it roll something the DM never asked for while looking exactly as if it had. An _unconfigured_ card falls to the first table by name, so a freshly added card works before anyone opens the gear; that fallback is a default, not a choice, which is why the settings picker shows empty rather than pre-selecting it.

The result panel is `components/dungeon-features/RollTableResult.vue`, extracted from `RollTableDetailView`'s roll panel. Shared because the encounter jump is conditional — an entry may or may not name an encounter, and a second copy that forgot the `v-if` renders a link to `/encounters/null`. The Roll _button_ is deliberately not in it: the detail view disables its own on a range error computed from an unsaved form, and the widget has no form. Each owns its trigger; the component owns the answer. The last roll is a local `ref` and never layout state — otherwise the dashboard reopens tomorrow still claiming the party met six goblins.
...
Advantage applies to the d20 only — 5e ties it to checks, saves and attacks, and "roll two d6 and keep the higher" is a house rule, so offering the toggle on every die would imply a mechanic the game does not have. The toggle nevertheless stays _visible_ for all dice rather than appearing and disappearing per click, and `effectiveMode()` collapses it to normal for a non-d20 so the result never labels itself with a mode that did nothing. `src/lib/dashboard/quickDice.ts` holds the button list, that rule, expression validation (`empty` is its own state, not an error — an empty field is the resting state, not a mistake) and the reshaping of both roll paths into one display shape.

**Death saves (`DeathSavesWidget.vue`)** — the third self-hiding widget, and the one whose absence _is_ the feature. Picker-only, `maxInstances: 1`, `selfHiding: true`. It does not exist on the page until someone is down; a card that sat there saying "nobody is dying" would teach the DM to stop reading it.
...
**Upcoming events (`UpcomingEventsWidget.vue`)** — the next few events on the _in-world_ calendar with a countdown for deadlines, which is a different question from `NextSessionWidget` ("when do we next play", a real-world date out of `session_proposals`). Picker-only, `maxInstances: 1`.

**`src/lib/calendar/upcoming.ts` is a new folder, and the arithmetic in it is the reason.** No next-N query existed anywhere, and the three ad-hoc day-ordering implementations that did exist are each wrong for this purpose and were deliberately not copied: `CalendarTimeline.vue`'s `eventToFrac` flattens every month to a nominal 30 days (fine for a pixel ruler, wrong for a countdown); `useCalendarEvents`'s Supabase `.order()` chains sort festival days before month 1 regardless of their real `afterMonth`; and `CalendarGrid.vue:268-275` hardcodes a Gregorian February rule. `upcoming.ts` instead builds a real per-year day index from `CalendarAdapter.months` / `.intercalaryDays` / `.isLeapYear`, handling two intercalary days sharing an `afterMonth`, `isLeapOnly` gating, and true cross-year arithmetic — an event in the first month of next year must sort _after_ the last month of this one, which is what a naive `year * 365` gets wrong. Thirteen tests cover exactly those edges.
...
Its four states are spelled out rather than derived from `groups.length`, because three of them look identical if you only count: below two characters the query is _disabled_ and answers with its placeholder `[]`, which is the same empty array a genuine no-match returns and the same one an in-flight request has not replaced yet. "Type more", "searching" and "nothing matches" are three different things to say. The search box is a local `ref` — it filters a popup of candidates, which is the sanctioned add-picker shape, the same one `GlobalSearch` itself has.
...
**Rule tracker (`RuleTrackerWidget.vue`)** — a homebrew rule's stateful tracker (Sanity, Exposure, Corruption…) live for the whole party, with its DM buttons. The second **configurable** widget, `maxInstances: 4`, and `src/lib/dashboard/ruleTrackerCard.ts` is `rollTableCard.ts`'s shape again: three states, and a `missing` card that says so rather than showing another rule's tracker. It has one extra failure mode — a rule that still exists but _lost its tracker_ resolves to `missing` too, since there is nothing left to draw.
...
Nothing in the app turned a transition into a sentence, so `questActivity.ts`'s phrasing is new rather than a duplicate — the cockpit's own path rendering (`QuestRunStorySoFar.vue`, which replaced `QuestRunPath.vue` in the Quest Manager Redesign) reads a different, narrower shape (`QuestRuntimeContext.path_so_far`). The table denormalises `from_quest_title` / `from_beat_title` / `to_*` at write time (`20260810000007_atomic_quest_runtime_navigation.sql:34-53`), so no beat fetch is needed. The null pairs are not defensive padding: only `start` leaves the `from_*` columns null (kind `enter`) and only `end` leaves the `to_*` null, and table check constraints guarantee each pair is null-null or both-present.
...
**No mechanism exists to focus one rule in the compendium**: `CompendiumTab`'s `selected` is a plain local `ref`, wired to neither URL nor store. What _is_ store-backed is `ui.compendiumSearch`, the tab's own filter. So a result sets that to the rule's name and pushes `/rules?tab=compendium` — it does not select the rule, but it narrows the destination to (usually) just that entry. A real deep link would need `CompendiumTab` to read a query param.
...
**It matches `fireDueTriggers`' day math, not `lib/calendar/upcoming.ts`'s, and that is deliberate.** `useQuests.ts:44-47` uses a private helper with fixed 12×30-day months that ignores intercalary days; `upcoming.ts` is adapter-aware. The adapter version is more nearly _correct_, and using it here would make the widget disagree with the code that actually fires the trigger — a card saying "in 2 days" for something that fires in 3. A widget must agree with reality before it agrees with the calendar. The underlying inconsistency is filed rather than silently changed, since fixing it moves when triggers fire.
...
**#764 expected this to need a schema addition — "the inverse of `player_read_items`" — and it does not.** That table is keyed `(user_id, campaign_id, entity_type, entity_id)` with four plain `auth.uid() = user_id` policies; nothing in it is player-specific except its name. The DM is an authenticated user who owns the campaign, so `useReadItems("journal_entry")` _is_ the DM-has-seen marker, with no migration and no new RLS surface. The table's name is now a little wrong — it is a per-_user_ read marker — and that is recorded here rather than migrated, since renaming it would touch every existing row and reader for no behavioural gain.
...
**That one hid because both halves used the same wrong helper.** `scheduleQuestTriggers` stamped a fire date with it and `fireDueTriggers` compared against it with the same function, so they agreed with each other perfectly and nothing looked broken. It only surfaced when the #764 dashboard card computed the same date correctly and disagreed — which is why that widget originally _mirrored_ the wrong formula on purpose.

Two decisions worth keeping:

- **`addDays` never returns an intercalary day.** `quest_trigger_scheduled` stores three integer columns and has nowhere to put "Shieldmeet", so an offset landing on a festival rolls forward to the next dated day. Intercalary days still _count_ as elapsed — ignoring them is the bug.
...
- **`NextSessionWidget`** — nearest non-cancelled `session_proposals` row, shown as a countdown. The deadline is the point: gaps matter _because_ Thursday is coming.
...
`QuestsWidget` had three predecessors — _In progress_ (chains with a live cursor), _Active Quests_ (the kanban lane) and _Rumors_. All three read the same data, each stood mostly empty, and a quest being played appeared in two at once.
...
- **Grouping** by stage under headings looks right until quests are properly built out — at which point every active quest holds a cursor, so _Party is here_ and _Active_ list the same quests under two headings, and the heading repeats what the row's own badge says.

So `lib/dashboard/questRows.ts` merges every reading into **one row per quest**, sorted `here → paused → active` (the `rumor` stage went with the quest-level Rumor lane in #874 — a rumour is a beat's visibility now, not a place a quest can be), and the widget renders a flat list. A cursor always wins over the lane: where the party is standing is the strongest thing you can say about a quest, and it carries the beat, which beats the giver at a glance. `DashboardQuestRow` never changes shape between stages, so only the dot and the trailing label differ.
...
8. **Chronicler AI image generation** (DM-only, shown whenever `campaign.activeCampaign.image_provider` is configured — `hasImageProvider`). Works with every provider we expose (OpenAI, Google Gemini) on both the server-side edge-function path and the BYOK local-vault path, which routes through the shared `getImageProvider()` abstraction (`src/ai/providers/`):
   ...
   **Where "at the cursor" actually is** — `insertionPos` (`src/lib/tiptap/insertionPos.ts`), which every `insert*AtCursor` on `RichTextEditor` routes through. ProseMirror's selection starts at the top of the document and stays there until something moves it, so an editor nobody has clicked into reports position 1 — indistinguishable from a cursor deliberately at the top. Reaching for a toolbar button without clicking into the prose first (the ordinary way to use a toolbar) therefore dropped chronicles, images and pending-image anchors _above_ the DM's own notes. `RichTextEditor`'s `onFocus` flips a `cursorPlaced` flag; until it does, insertions append to the end. The same helper steps out of an enclosing `aiGenerated` wrapper, so a second chronicle sits beside the first instead of nesting inside its provenance markers.

9. **Chronicler AI text generation — "Write Chronicle"** (DM-only, `ai_enabled`- and credit-gated in the UI — not plan-gated, runs on any plan per the 24 Sep 2026 policy — toolbar button when `hasTextProvider`): `ChroniclerWriteDialog.vue` takes raw session facts (a `MentionTextarea`, `@`-mentions client-resolved to party-member/NPC/monster/faction descriptions) plus a tone, and `useChroniclerTextGeneration` returns markdown that "Insert into Note" splices into the currently-open note — there is no chronicle table; chronicles ARE note content. Server path = `generate-chronicle-text` (credits key `chronicle_text`); local-key mode keeps the ungrounded client path (policy — see `useQuestGeneration.ts`). The function deliberately does NOT use `_shared/textGen.ts`'s `callText`: that dispatcher forces JSON-mode output, which commit `97261fe1` removed from chronicles because constrained decoding let an unescaped quote silently truncate the narrative — the transport stays local until `callText` grows a plain-text opt-out (documented in the function's module doc).

   **@mentions are the image anchors, so the Chronicler writes them.** `parseSceneEntities` (`src/ai/sceneEntities.ts` — the parser moved out of `useChroniclerImageGeneration.ts` when the soundboard's AI music form started reusing it) turns each `@Name` in a scene prompt into a reference image — a portrait, a location's `image_url`, the party's group portrait for `@Party`, or a faction's `emblem_url` (rendered as heraldry, never as a character; both `buildScenePrompt` copies say so) — which anchors a likeness far better than any description. The `chronicle_text` prompt used to forbid names in `[[scene: …]]` suggestions ("describe visually instead"), throwing that anchor away; since `20260924195100` a suggestion @mentions everyone present, in the same `@Underscored_Name` form the prompt box's @-picker inserts, and only names from the Entities list or the retrieved campaign context. Retrieval is what makes that work without the DM @mentioning everyone in the facts: it surfaces related NPCs and factions by exact name.

   **Tags.** The chronicle ends with a `[[tags: …]]` line. `parseChronicleTags` (`src/ai/chronicleTags.ts`) strips it like `parseChronicleHeading` strips the title; `reconcileChronicleTags` rewrites a tag that already exists (by `normalizeTag`, `src/lib/tags.ts`) to its stored spelling, since stored tags mix `icewind dale` and `icewind-dale`. The prompt's `{existingTags}` placeholder carries the campaign's note tags, most frequent first, so the model reuses rather than coins. The preview offers them in an editable `TagInput`, and Insert merges them into the note's tag bar.
...
type JournalCategory =
  "adventure" | "clue" | "discovery" | "session" | "character" | "rumor";
type JournalRefType =
  "quest" | "npc" | "location" | "item" | "monster" | "encounter";
