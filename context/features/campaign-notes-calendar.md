...
**Known consequence:** Customize mode opens the row gap (`gap-y-12`, `md:gap-y-8`) to make room for the control pill, so a spanned card is a little taller in that mode than outside it. Since #763 that gap was the only visual difference between the modes; with fixed row heights it now also changes card height slightly. Tolerable because a height is chosen as a *number* — the pill shows "2 of 4" — rather than eyeballed in pixels, so the mode never misrepresents the choice being made. Moving the pill inside the card is not the fix: it covers `DashboardWidget`'s own "View all →" link, which is why it floats in the gap at all.
