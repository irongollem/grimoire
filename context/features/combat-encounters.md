...
**`seededForLive`** (`EncounterMapView.vue`) is the flag that keeps the fog seed a one-time event per go-live, rather than inferring "not yet seeded" from an empty mask the way an earlier pass tried. That inference broke on its own success: a DM's own "Hide all" action *produces* an empty mask, and inferring from emptiness meant the next surface recompute (regions refetching, etc.) would silently re-reveal it, undoing the DM's own action. `seededForLive` resets only on the false→true transition of `isLive` — a genuine new go-live — so an unrelated recompute can't re-trigger it. Token seeding needs no equivalent flag, because `seedTokenPositions()` is idempotent on its own (it only ever fills combatants with no position), so it simply re-runs on every call.
