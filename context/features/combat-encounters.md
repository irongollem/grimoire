...
**Copying to another campaign (#598).** Re-scoping *moves* the row; copying makes a second, independent one in a campaign the account DMs, reachable from the Select surface's bar and from the detail editor's own actions. The rules — which cross-entity references travel, which are dropped and named, which are reported rather than dropped, and why the copy neither suffixes its name nor navigates afterwards — are written once in `items-spells-crafting.md` ("Copying to another campaign"). Monster-specific notes: `lair_location_id` is cleared when the lair belongs to a campaign the target cannot see; the copy is queued for embedding like any new monster; and `monsters` is one of only two of the eight tables carrying `enforce_quota`, so an over-limit copy opens the editor's existing paywall. The action sits beside Duplicate on the desktop action bar and in `MonsterEditMobile`'s overflow sheet — both, deliberately, so the two surfaces of one editor cannot drift apart.
...
**Getting the corpus embedded in the first place.** `supabase/functions/embed-monsters/index.ts` is the endpoint that actually writes embeddings, in two modes: `mode: "batch"` (admin-only, driven in bounded pages of 100 rows per call by `useEmbeddingBackfill.ts` — see Switching vendor below) and `mode: "single"` (fire-and-forget after any monster create/update, via `queueMonsterEmbedding()` in `src/composables/monsters/useMonsters.ts`, so a DM's save never waits on an embedding call to finish; and **awaited**, not fired-and-forgotten, after a `create`/`update` through the MCP server — `queueEmbedding()` in `_shared/mcp/tools.ts`, driven by each entity's `embedOnWrite` in the MCP registry, because an edge isolate may be torn down the moment it returns and would drop an unawaited request, leaving a hook that looks wired and does nothing). Both build the embedded text from one shared function, `buildMonsterEmbedText()` (`supabase/functions/_shared/monsterEmbedText.ts` — name, then size/type/CR, then tags, then habitat, then a word-boundary-truncated description, each clause omitted rather than left dangling when its source field is empty), so `monsters` and `library_monsters` rows land in the same region of the embedding space and stay comparable. A SHA-256 hash of that text (`monsterEmbedHash()`) is stored as each row's `source_hash`; a row whose hash and `embedding_model` are unchanged is skipped, so an edit that didn't touch anything semantically relevant costs no provider call.
...
**Creating the encounter.** "Create Encounter" persists the result — name, matched combatants, default factions, and every current party member **and companion** auto-selected (companions come along too, matching what `EncounterDetail` does for a hand-built new encounter — "start with the whole party, bench whoever shouldn't join"; they also count as allies in the difficulty calculation, so leaving them out would make the generated encounter read as harder than the identical one built by hand) — and the button becomes "Open Encounter →", which navigates straight into the **editor** (`/encounters/:id?edit=true`), not the read-only sheet. It does not auto-navigate on create; only "Open Encounter →" does. A generated encounter is a draft: the DM still has to review the AI's picks, swap what doesn't fit, and hand-add anything that wasn't in the Bestiary — all edit-mode work, unlike the Roll Table Generator's create-then-"View Table →" link, which points at a view because a generated table is finished on arrival and this isn't.
...
- **`turn_timer`** — a _configurable_ rule (the first one). The DM sets **seconds per turn** (default 60, clamped 5–600). Config is the general mechanism added here: `campaign_rules` gained a nullable `config jsonb` column (migration `20260724000006`), `OptionalRuleDef.config?: RuleConfigField[]` declares the tunable fields, `CampaignRule.config: RuleConfig | null` holds the stored values, and `resolveRuleConfig(rows, key)` (`useOptionalRules`) merges stored values over registry defaults (dropping any non-numeric stray). `RulesTab.vue` renders a number input per field only while the rule is enabled; both the toggle and config writes persist through the single `useUpsertCampaignRule` mutation hook (config optional — flipping the switch passes the existing `config` through so it never wipes tuned values). `campaign_rules` is in `useCampaignLiveSync`'s `SYNC_TABLES`, so a DM toggling/tuning a rule reaches already-mounted players live. The countdown itself is `TurnTimer.vue` (`src/components/encounters/`) — a purely-visual, self-contained clock taking `:seconds` + `:reset-key` (round + active combatant instance id). It restarts whenever the key changes, flashes amber ≤10s, pulses red at 0, and never force-ends a turn. Rendered in the runner top bar (`EncounterRunner.vue`, gated on `store.round > 0`) and on the player panel (`PlayerEncounterPanel.vue`, gated on `!isInLobby`); both derive `turnTimerSeconds`/`turnResetKey` from the shared `useTurnTimerConfig(round, activeCombatantId)` composable (`src/composables/encounters/useTurnTimerConfig.ts`) — only the round/active-combatant source differs (store vs. liveState). Each side runs its own copy — a sub-second drift is fine for a nudge, so no start-timestamp is synced.
...
**Chat mode bar**: toggles whether rolls are posted to campaign chat — Public (roll result visible to all in chat) / Silent (roll not posted to chat).
...
## Battle Maps — The Room Bridge

Before #868, opening the battle map required a location with its own uploaded image and an explicit `is_battle_map` toggle — a room with no separately uploaded art simply had no battle map to open. `src/lib/battlemap/roomBridge.ts` (epic #868, frame 13 "When the plan is a battle map") bridges this the other way: a room traced on its site's *published* Atlas plan is already a legal battle map at combat's own cell scale, with no separate art, no separate calibration, and no toggle to remember.

**Two coordinate spaces, one converter pair.** A room's traced cells (`location_map_regions.cells`) live in the Cartographer's own map-cell space — the same space the site's structure was published into (dungeon-craft.md's "Placed in" section covers the sibling case for placements). The battle map's tokens and fog mask, however, have always lived in a separate *image*-cell space, where cell `(0,0)` is the baked image's own top-left corner. The two spaces are offset by exactly the padding the bake added around the drawing, recorded as `grid_calibration.origin_cell_x/y`. `regionCellToBattleCell()` / `battleCellToRegionCell()` (`roomBridge.ts`) are the only two functions that ever apply that offset — every room outline, zone overlay, token seed and fog-reveal check goes through one of the two, so a mismatch there would silently misplace every token seeded into a room rather than surfacing as an error.

**`resolveBattleSurface()`** decides which map an encounter's battle view actually draws, given the encounter's own location, that location's parent, and the parent's regions. Three outcomes: a location with its own `map_url` **and** `grid_calibration` behaves exactly as it did before #868 — nothing changes for a location that carries a separately uploaded battle image, room or not; an **interior space** (`room` or `grounds` — `isInteriorType()` from `@/lib/locations/tiers`, since #886 moved `grounds` down from the site tier to sit beside `room`) with no map of its own but a calibrated parent site resolves to the *parent's* plan instead, focused on whichever region has `region_role === "space"` and `space_location_id` equal to its id (its cells converted to battle-cell space); anything else — an uncalibrated site, an interior space with neither — resolves to `null`, meaning there is no surface to draw. The result (`mapLocation`, `focusRoomId`, `focusCells`, `calibration`, together a `BattleSurface`) is the single source of truth the runner's gate and the map view's render both consult, via `useLocationBattleSurface()` / `useEncounterRoom()` (`src/composables/encounters/useEncounterRoom.ts`) — both call the same function with the same inputs, so they cannot disagree about which map is on screen. Every call site that decides "does this location ride its parent's plan" goes through `isInteriorType()` rather than comparing `location_type === "room"` inline, so `grounds` is never silently excluded again.

**`seedTokenPositions()`** places every unpositioned player-type combatant — party members and companions alike, "party tokens" in frame 13 — onto a free cell inside the focus room, row-major and footprint-aware, never overlapping two tokens. It only ever fills combatants that have no position yet, which is what makes it safe to call on every mount while live rather than once: re-seeding after the DM has dragged tokens around is a no-op for the ones already placed.

**`seedFogMask()`** starts the fog mask with the room's own cells already revealed — "you are standing in it" (frame 13/16) — instead of a blank map the DM has to manually unveil before the party can even see where they're standing.

**`roomsRevealedInCombat()`** is the other end of that fog: at the end of combat it walks every `space`-role region and marks it "revealed in this fight" once at least half of its cells are in the fog mask (both sides compared in battle-cell space via `calibration`). That half-revealed threshold, not "any cell" and not "every cell", is what decides which rooms get offered for exploration marking below.

**`useCombatExploration()`** (`src/composables/encounters/useCombatExploration.ts`) owns the gate and the end-of-combat prompt, both extracted out of `EncounterRunner` when it grew past its line cap. The gate (`resolveBattleMapGate()`, pure and independently tested): a room riding its site's published plan is *already* a legal battle map, so `is_battle_map` is not consulted at all once `surface.focusRoomId` is set — being the focus of a resolved surface already proves the map is the site's own calibrated plan drawn at cell scale, not tactical art the DM has to separately opt into showing. `is_battle_map` still gates a plain, non-room-anchored location exactly as before: it remains the switch that hides tactical art from players everywhere else. Every other combination (no location linked, no map at all, an uncalibrated map) returns its own reason string rather than a single blanket "can't open."

At the end of combat, `markRoomsExploredFromFogMask()` asks **once** — "Mark N rooms revealed in combat as explored?" — covering every room the fog uncovered that fight, never a per-room decision. Confirming writes an `explored: true` fact via `useAssertLocationState` for each room. This is deliberately one-directional: nothing in this path, or anywhere else in the room bridge, ever un-explores a room by un-revealing fog — brushing a cell back to hidden mid-fight changes what the fog mask currently shows, not what the durable location-state log has already recorded.

**`shouldSeedFog()`** (`src/lib/battlemap/fogMask.ts`, called from `EncounterMapView.vue`'s `seedRoomIfNeeded()`) gates the one-time fog seed off the row's own `fog_mask`, not a component-local flag — an earlier local-ref "already seeded" approach (`seededForLive`) reset on every remount (a browser refresh, or the second window `openBattleMapInNewWindow` opens), so it went false again while the encounter was already live and replaced whatever the DM had revealed mid-fight with just the focus room. `fog_mask` is server truth instead: `goLive` (`useEncounterLive.ts`) writes it to `null` on every fresh go-live and nothing writes it back to `null` afterwards, so `shouldSeedFog(fog_mask)` — true only when `fog_mask === null` — answers "never seeded this fight" correctly across any number of remounts. It also tells a DM's explicit "Hide all" (an empty mask, encoded `""`) apart from "never seeded" (`null`), which the old local flag could not distinguish — that gap let a surface recompute quietly undo a DM's own "Hide all". Token seeding needs no equivalent guard, because `seedTokenPositions()` is idempotent on its own (it only ever fills combatants with no position), so it simply re-runs on every call.

**`BattleMapRoomFocusLayer.vue`** draws the visual half of a focus room: the whole plan dimmed to a translucent near-black everywhere except the focus room's cells (one SVG path with an even-odd fill rule, so the "holes" are the visible room), the room's own outline traced from its cells' actual boundary edges rather than a bounding box (so an L-shaped room reads as L-shaped, not as its rectangle), and any difficult-terrain zone inside the room outlined in dashed cyan. The zone outline is a *read*, not an enforcement — frame 13 is explicit that it "does not halve anyone's speed"; the DM still applies that by hand with the existing condition/movement tooling.

**`BattleMapToolbar.vue`** gained a Site/Encounter segmented control in its header (Encounter is always the selected value here, since picking Site is what navigates away to the Atlas). **`BattleMapLegend.vue`** is new alongside it: a bar under the map explaining the Party / Hostile / Large / Active-turn token colours — pulled from the actual rendering constants (`DEFAULT_TOKEN_RING_COLOR`, `DEFAULT_FACTIONS`) rather than a second invented palette — plus one row per difficult-terrain zone inside the focus room, named for that zone's own `label`.

**`EncounterBattlefieldSetup.vue`** — the builder's pre-placement panel, used before an encounter ever goes live — resolves readiness through the same `useLocationBattleSurface()` rather than checking the location's own `map_url` / `is_battle_map` directly: a room with no map of its own is "ready" for token placement the moment its site's plan is calibrated, exactly as the runner treats it, so the builder and the runner can never disagree about whether a given room supports placement.

**Still no VTT.** None of this makes token movement or fog brushing automatic, and nothing here is synced to anything beyond the DM's own strokes: a token seeded onto a room's cells does not move itself, revealing one cell does not trigger a reveal anywhere else, and the end-of-combat exploration prompt is a DM confirmation, not an inference the game draws on its own. The room bridge is prompts and coordinate translation, not a rules engine — the same discipline the site runner itself commits to elsewhere in #868 ("prompts, not automation"). It makes an already-published Atlas plan usable at combat's cell scale; it does not add anything that fires on its own underneath it.
