...
| Layer | Path |
| --- | --- |
| Migration | `supabase/migrations/20260925002215_demo_campaign.sql` |
...
- `campaigns.demo_template` (one row at most, by partial unique index), `campaigns.demo_version` (the template's published stamp) and `campaigns.demo_source` (on a copy: the version it came from; at most one copy per user).
...
| `get_demo_status()` | any DM | `{ published, version, demo_campaign_id, loaded_version }`. Never the template's id. |
| `publish_demo_version(p_campaign_id)` | admin, own campaign | Dry-runs the copy (always rolled back), then marks the campaign as the template and stamps a new version. |
...
- **Publish after editing.** New loads copy the template as it is *now*, so half-finished edits reach new users immediately. Publish bumps the version and lets existing copies offer a reset.
