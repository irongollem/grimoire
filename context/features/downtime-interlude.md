...
- **Phase 3 — `srd_*` seed library: deliberately NOT built.** The rule in this doc is that seeds become an `srd_*` table *the moment we ship curated per-seed artwork*. There is no artwork, and the procedural `accent`+`glyph` faces work. Creating the table now buys nothing and adds a migration + storage policy to maintain. Build it **in the same migration** as the first real art, not before.
