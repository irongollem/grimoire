...
**"Placed in" on the grid card itself** (#868, S8, frame 11). The per-entity "Placed In" panel above needs the DM to already be on a trap's own page; the Traps, Features and Puzzles tabs in the Dungeon Craft hub now carry a one-line summary — `<site> · <room> [cell x,y]` — directly on each grid card, via `usePlacedInRooms`/`resolvePlacedInRooms` (`src/lib/dungeon-features/placedIn.ts`) and the shared `PlacedInLine.vue`. It is a genuinely different query shape from the panel above, not a duplicate: the panel resolves one entity's placements, this resolves every visible card's placements in a single `.in()` query (`useEntityPlacementsFor` in `useLocationPlacements.ts`) rather than mounting one query per card in a `v-for`. `location_placements.source_cell_key` (migration `20260908215643`) is what lets the line say *which square*, not only which room: it is set when a placement was drawn on the site's plan through the Cartographer/Publish path, and left null when the placement was added by hand from the room sheet — in which case the line names only the room, never a cell. Puzzles key this composable on their *host feature's* id rather than their own, because a puzzle has no `location_placements` row of its own, and layer `directRoomPlacement()` (also in `placedIn.ts`) on top locally for a puzzle anchored straight to a room with no cell of its own.

Both the per-entity panel and the grid-card line invalidate the same two broad query-key prefixes on every mutation — `[..., "site"]` and `[..., "entity", <kind>]` (`invalidateBroadPlacementQueries` in `useLocationPlacements.ts`) — alongside the narrower single-location/single-entity keys the five placement mutations already invalidated. TanStack matches a partial key as a prefix, so one invalidation reaches every mounted grid and every batched site view without either mutation needing to know which grids or id sets happen to be on screen. A placement made through the Cartographer's publish, a room sheet, or a trap's own "Placed In" panel therefore shows up in the grid line without a reload.
...
### A door citing a feature

A "Secret Door" feature and a door drawn with `is_secret` used to be the same fact recorded in two catalogues, with the perception DC retyped as prose into the door's `lock_note`. `location_doors.dungeon_feature_id` (migration `20260908215641`, #868) closes that: a door may cite the Secret Door, Hidden Passage or Moving Wall feature that conceals or opens it, and `DoorFeatureCard.vue` reads that feature's own `perception_dc` / `investigation_dc` / `arcana_dc` / `trigger_type` from where it was authored instead of a second, driftable copy.

This is a **link, not a merge**. Merging would have meant folding `dungeon_features` into `location_doors` — but only three of the seven feature types (Secret Door, Hidden Passage, Moving Wall) are connections at all; a Treasure Chest, Hidden Cache or Concealed Alcove has nothing to do with a way between two rooms, so collapsing the two tables would be right for three of seven types and wrong for the other four. The nullable FK keeps them as two independent rows with one honest relationship instead: a door may cite a feature, a feature never requires a door, the feature still lists on the Features tab with its own DCs and trigger, and the door still lists on its room's Ways Out panel with its own `label` / `is_one_way` / `starts_locked`. `on delete set null`, never cascade — deleting the catalogue fixture must not delete the connection in someone's dungeon.

The `guard_location_door_endpoints` trigger enforces that a cited feature is the door author's own: the RLS insert policy on `location_doors` only checks the door's originating location, and a feature id supplied on the row is otherwise a free uuid a DM could point at someone else's feature.

This is the same column documented from the Atlas / door-editing side in world-building.md; this section covers it from Dungeon Craft's own feature-authoring perspective.
