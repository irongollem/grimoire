...
- Scope — "All scopes" (default), "This campaign", "General", "Library", plus "Other campaigns" once "Show items from all campaigns" is on. Classification is `itemScopeOf()` (`src/lib/items/itemScope.ts`): a non-UUID id is always `library` regardless of its stamped `campaign_id` (`normalizeLibraryItem` stamps `campaign_id: null`, the same value a real `general` row carries, so id shape — not scope — is what tells them apart); a UUID row is `general` (`campaign_id IS NULL`), `campaign` (`campaign_id` = active), or `other_campaign` (any other campaign). "Other campaigns" is hidden from the select, and the stored filter reset to "", whenever the all-campaigns checkbox is off — `useUiStore` watches the checkbox for this so the two can't drift.
