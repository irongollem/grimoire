...
- **Weapon section** (appears for weapon types) — damage rolls (multi-roll via `DamageRollsInput`), versatile damage (dice expression), range (e.g. "80/320 ft."), weapon properties checkboxes (finesse, light, thrown, two-handed, reach, heavy, loading, silvered, adamantine, etc.).
...
- **SRD + Open5e import pipelines** — one-click import for items (Open5e API + local datasets) and spells (selective by sourcebook). Both are upsert-safe and preserve user-customized data.
