...
| Generate (AI, Lyria)   | `upload`      | **Yes**                                           | Two-step: `structureMusicPrompt()` expands the description, then Lyria generates audio. Server/key-vault work is a durable job: the edge worker uploads audio before the client creates the sound, preserving the original campaign/page if navigation happens. Local-BYOK remains client-side and is saved through `useSoundUpload`; both are saved with `artist: "Grimoire AI"`. |
