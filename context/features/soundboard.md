...
`src/lib/audio/aiMusic.ts` exports `structureMusicPrompt` (expands the DM's request into a complete Lyria prompt via a text model: musical direction, a timestamped timeline ending at the requested length, verbatim lyrics under `Lyrics:` — written to Google's Lyria prompt guide), `composeFallbackPrompt` (used when the text model is unavailable), `generateMusicLocally`, `MUSIC_LENGTHS` and `LYRICS_MAX_CHARS` (2200). The structuring system prompt lives in `ai_system_prompts` as `music_structure` and in code as the fallback `MUSIC_STRUCTURE_SYSTEM`; `aiMusic.test.ts` fails if the two diverge. The prompt that comes out of structuring is sent to Lyria as-is — lyrics are never re-prepended (they were, until the 3.5 upgrade, so every lyric went out twice). `supabase/functions/generate-music/index.ts` is the server-mode path: validates campaign membership, resolves BYOK-vs-platform Gemini key (BYOK is Pro-only per `project_byok_pro_only.md`), reserves AI credits, creates an `ai_generation_jobs` row, and returns its id immediately. Its background worker uploads the audio to the `sounds` bucket and marks the job ready **before** recording billing; `SoundForm` waits on that row and creates the sound from the durable URL/path and original campaign/page metadata. The client-side `generateMusicLocally` path is used instead when the user has a **local-vault** BYOK key (`grimoire_key_local_mode === "local"` in `localStorage`); local-key work remains browser-owned and cannot survive a reload mid-generation.
...
| Generate (AI, Lyria)   | `upload`      | **Yes**                                           | Two-step: `structureMusicPrompt()` expands the description, length and vocals choice into a full prompt, then Lyria 3.5 generates audio. Server/key-vault work is a durable job: the edge worker uploads audio before the client creates the sound, preserving the original campaign/page if navigation happens. Local-BYOK remains client-side and is saved through `useSoundUpload`; both are saved with `artist: "Grimoire AI"`. |
