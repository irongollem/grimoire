...
- Filter bar: text search (title, summary, tags), a **Shared with party** toggle, and a searchable NPC/location facet. Filter state uses the quest-specific `useLocalStorage` entries in `useUiStore`, so it survives both navigation and reload; **Clear** resets the composed state. Entity matching includes the primary giver/location plus typed `quest_refs`, loaded once per campaign by `useCampaignQuestRefs` rather than once per card. Factions are deliberately absent until quests gain an authoritative faction attachment (#660); prep-gap and pending-loot filters are already represented in the pure `filterQuestBoard` contract but remain hidden/no-op until beat summaries (#658/#661) exist.
