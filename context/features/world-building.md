...
**`LocationDetailSections.vue` — shared by the sheet and the pane.** Holds the seven sections that are *about* a place rather than *where it sits*: Description, Related Locations, Store, Rooms, People in the Area, Encounters Here, Currently Here. Self-contained (does its own queries; TanStack dedupes against the caller's). Each caller keeps what genuinely differs — breadcrumb, identity, sub-locations, map placement, and the Atlas's scale rail and tier groups.
