...
Route: `/locations` (list, and every place — see `placeRoute()` in `lib/locations/placeRoute.ts`), `/locations/new`. A place opens at `/locations?at=<id>`, with `?edit=true`, `?build=true` or `?run=true` switching the Atlas pane into its Details, Build or Run state. `/locations/:id` still resolves — it redirects to the Atlas convention above, carrying its query along, so an old bookmark or link still lands somewhere real.
...
It was extracted because a now-removed full-page sheet (`LocationSheet.vue`, see "One DM screen" below) needed the same sections the Atlas pane already had, and a second copy drifts within a release: the sheet's People card grid and the pane's People list were already two designs for one thing before the extraction. `LocationSheet` went 403 → 217 lines at the time; the Atlas pane is now this component's only caller.
...
The map itself — and the room-shapes list that rides alongside it — is deliberately **not** one of these six: #807 removed the `SiteMapView` panel this section used to list here, because it rendered `map_url` a second time. Map placement genuinely differs per caller (the Atlas pane's mode toggle, the run surface's composed layout), so `LocationMap.vue` — the caller-owned composite — carries the regions canvas and `SiteMapRegionList` with it instead. See "Clickable rooms on a site's map" below.
...
**Details/Edit, Build and Run are all states of this one pane, not trips to another page.** Since the pane renders the same body a separate page would, a link to that page always led where the reader already was. All three buttons push a query flag onto the current Atlas selection (`?edit=true`, `?build=true`, `?run=true`) and `AtlasExplorer` swaps `AtlasPlacePane` for `LocationEditor` or `SiteRunSurface` accordingly — the one thing the pane itself cannot do is change the place, which is what Details/Edit is for.
...
**A place has one DM screen now (25 Sep 2026).** It used to have two: this Atlas pane at `/locations?at=<id>`, and a separate full page at `/locations/:id` (`LocationDetailView.vue` + `LocationSheet.vue`/`LocationEditor.vue`, all now removed). Two screens for one place meant they could disagree, and did: saving an edit made on the Atlas landed the DM on the other screen — an older, different-looking page they had not asked to visit — and that page's ambience preview kept autoplaying regardless of what the Atlas pane was doing. `LocationEditor`'s save, create and delete now land on `/locations?at=<id>` (delete: the parent, or the bare Atlas) because the Atlas is the list and the list is the confirmation; Cancel drops whichever mode flag it was showing and stays on the same place. `/locations/:id` still resolves, as a redirect to the Atlas convention, so nothing that linked to it breaks.
...
**A place's working states, all inside the Atlas** (`AtlasExplorer.vue` + `LocationEditor.vue` / `SiteRunSurface.vue`)

Editing follows the pane + editor convention: the Atlas pane is the read-only view; adding `?edit=true` to the current selection swaps it for `LocationEditor`. Creating a new location is the one thing that still needs its own page (`/locations/new`, `LocationCreateView.vue`) — there is no place yet to select onto — and goes straight into the editor. A breadcrumb showing the full ancestor chain (Locations → Parent → … → Current) is shown in the editor; the pane has its own clickable-ancestor breadcrumb above the map.

**A site has two working states, and Run is a layout (epic [#884](https://github.com/irongollem/grimoire/issues/884), wave 2).** A site-tier place used to edit two ways at once: the record behind `?edit=true` with Save/Cancel, and rooms, ways out, zones, placements and ambience written *live* by always-editable panels that rendered in read mode — so the one button saying "Edit" governed a form the DM was not in, and arriving from a quest beat put them in a surface where everything was already editable. Now:

- **Browse** is the default and where a beat's "Open in Atlas" lands. Only play actions are live — Explored/Cleared/Looted, loot, reveal, store restock, clicking a space to enter it. The spaces and zones lists still *read* (names, counts, click-to-highlight, "Not traced"); what disappears is every affordance that changes the plan.
- **Build** (`?build=true`) is a superset: add/rename/reorder/delete rooms, ways out, zones, placements, ambience, Trace/Bind — all written live, so there is no Save. It is a state of the **Atlas pane** as well as the detail page (`/locations?at=<id>&build=true`), because the Atlas is the site's workbench and sending the DM to another page to build would reintroduce the round trip this epic removes.
- **Run** (`?run=true`) is unchanged and gates nothing of its own: "easy access to the things you need during gameplay, not a replacement for certain interactions".

**Build's map area is the Cartographer (epic #884, wave 4).** In Build, `AtlasSiteMapMode` mounts `MapWorkbench` instead of `LocationMap`: the DM traces spaces and zones, places doors, claims floor regions and paints tiles with one tool, one gesture set and one undo, without leaving the Atlas. Browse and Run still render `LocationMap`/`MapRegionsLayer` — display, hover, click-to-select, click-to-descend, click-to-move-party — and those components no longer carry any editing path of their own, because keeping a second implementation of tracing is precisely the legacy the golden rule forbids. The Drawing autosaves; there is no Save in Build. See cartographer.md for the palette, the layer selector and what the retired zone layer took with it.

**Fog is a layer, and it shades the plan (decision 7).** A hidden cell is fully black edge to edge, and the feather lies **outside** the black, on the revealed side — so nothing hidden is ever partly shown, and there is no light model in this pass: no walls or doors as blockers, no line of sight, no viewing angles. It is a visual effect over a per-cell mask (`fogMask.ts`'s feather, `siteFog.ts`'s room-resolution mask, `planCanvas.ts`'s `drawFogPass`), used at cell resolution by the encounter battle map and at room resolution by a site. The DM sees it translucently over the real plan — a hint, never a wall — and the player's own view is opaque. Line-of-sight fog is deferred, not rejected: the revealed set is a plain cell set and the renderer does not know how it was computed, which is what keeps that cheap to add.

Players receive the picture whole, behind client-side fog. That was a deliberate ruling — "it's a game, not a hospital or a bank" — reversing frame 16's withheld-picture rule. What is still withheld is *structure*: `get_player_visible_site_state` composes the plan from cells, so unexplored rooms, secret doors and lock notes never reach the client at all.

**A door is an edge, and its endpoints are derived (epic #884, wave 3, migration `20260915070839`).** A way out used to be made from a form — pick the other room, set the flags — so it had a relationship but no position, and #879 said so plainly: "I defined the doors, but I can't set a door on the minimap at all." Now the door tool in Build snaps to the nearest cell edge (the same 25% band the Cartographer's own wall and door tools use, shared rather than reimplemented), and a click there writes the `edge_key`. The two cells either side of that edge are looked up in the site's traced regions and *become* the endpoints: draw on the edge where room A meets room B and the app knows it joins A and B. Clicking a placed door cycles `door` ↔ `arch`; alt-click or right-click removes it; a stair is inferred when one side is a nested site.

`resolveDoorEndpoints` (`lib/locations/doors.ts`) is the one resolution, shared with the Cartographer's `deriveWays` so a published door and a hand-drawn one cannot disagree about which cell owns which side. Three consequences worth knowing:

- **A door with a region on one side only is kept, not refused** — `to_location_id` is nullable now. It reads "leads nowhere yet", draws dashed, counts as a readiness gap, and is skipped by run-mode reachability, because a party cannot walk through it. It is the entrance a dungeon is entered by, and the passage nobody has drawn yet.
- **It resolves itself.** Every region mutation reconciles this site's placed doors against the regions now beneath them (`reconcileSiteDoorEndpoints` in `useLocationMapRegions.ts`), writing only the doors whose endpoints actually changed. Trace the far room and the door stops leading nowhere.
- **An unplaced door is a gap, not legacy.** Doors made before this have no edge key; their row says so and offers "Place it", which arms the tool with that door so the next edge click positions it. Nothing was migrated, and the button only appears where a plan is actually on screen to aim at.

**The Layers panel** (`SiteMapLayersPanel.vue`) is where a site's map is chosen in Build: one row per layer of the stack, bottom-up — Picture (upload, replace, calibrate, remove), Drawing (open it in the Cartographer, style it with AI, or start one named after this site; its staleness reads here), Plan (what is traced, or "Start a blank grid", which writes `plan_size`). An empty row is an invitation, not an error. Removing the Picture clears `grid_calibration` with it — that calibration is pixel coordinates on *that* image and means nothing without it, while a Drawing carries its own. The panel deliberately does not name the picture's file: an upload is stored under a generated id, so a file name here would print a uuid at the DM; what it says instead is whether the grid is set, which is the thing they can act on.

**Style with AI (Drawing row) flattens the stack rather than adding to it.** Restyling composites the Picture underneath and the Drawing's live bake on top — the same two layers `MapStackImage.vue` already shows the DM (`placePicture`, the transparent bake) — and the result replaces the Picture, a best-guess `grid_calibration` derived from the composite's own fitted geometry (padded to the AI provider's accepted aspect window, at a resolution matched to a TV-sized pixel budget), and unlinks the Drawing (`source_map_id: null`) in one atomic write, `useSaveStyledSitePicture`. The `dungeon_maps` row itself is never deleted: it survives as an ordinary, standalone map in the Cartographer, so the DM can draw corrections over the new Picture (shown as `MapWorkbench`'s reference ghost, aligned by the calibration this save just wrote — see cartographer.md's M8 section for why a *fresh* drawing lines up with it too) and republish later, which puts tiles back on top. See cartographer.md's M8 section for the full field list, the sizing argument behind the calibration, and what currently shows when a Picture is styled while a Drawing still covers it.

One prop carries it — `building`, threaded from the route flag through `AtlasPlacePane` → `LocationDetailSections`/`AtlasSiteMapMode` → the structural panels and `LocationMap` → `SiteMapRegionList`/`SiteMapZoneList`/`MapRegionsLayer`, where disarming `activeRegion()` disables the paint/pen/template gestures at their source. Play panels (`LocationStateControls`, `LocationLootPanel`, `StoreInventory`) never take it. The site's action bar reads **Build/Done · Run · Details**, where Details is the old Edit — and on a site that form no longer owns the map at all, pointing at Build instead. Non-site places keep plain Edit: a village has no workbench.

**Location editor fields:**

- **Name** (required), **Type** (one of 19 types, see below), **Sigil/Emblem image** (portrait aspect, uploaded to `location-images` bucket)
...
**Atlas pane (view mode) — read-only sections:**

- Sigil + name + type badge + tags
- Description (only rendered when Tiptap JSON has text content)
- Interactive map with pins (clicking a pin selects the child location)
...
**Location type taxonomy** (19 types). Two independent axes — do not conflate them:
...
- *Concrete place types*: City, Town, Village, District, Building, Dungeon, Wilds, Wilderness, Other
- *Store types* (support inventory): Store, Tavern, Inn
- *Interior spaces* — excluded from pinning entirely (#807; widened by [#886](https://github.com/irongollem/grimoire/issues/886) when `grounds` joined `room` at the `interior` tier). `getPinnableDescendants()` skips an interior child outright (`isInteriorType()`, not a literal `=== "room"`) rather than offering it as a pin candidate: an interior space is placed by a traced region on its site's floor plan (`location_map_regions`), never by a pin. This is a pinnability exclusion, not a recursion one, so it lives beside the `VAGUE_LOCATION_TYPES` walk rather than inside that set — an interior space is already a concrete leaf, just not a mappable point on this axis.
...
| `district`   | District                                |
| `site`       | Building, Dungeon, Store, Tavern, Inn, Wilds |
| `interior`   | Room, Grounds                           |
| *(none)*     | Other — the escape hatch claims no tier |

`venue` is gone: a tavern is a building, one of the six types with a floor plan. `wilderness` moved to `land`, beside continent/region/country, where "no floor plan, children placed by pins" actually describes it. `district` earned its own rung: its children are buildings on a geography map, not traced rooms.

**`Wilds` vs `Wilderness` — the distinction a reader will get wrong ([#886](https://github.com/irongollem/grimoire/issues/886)).** They sound like synonyms and are not the same tier. `wilderness` is untouched by #886: it stays a `land`-tier pin map (#810) — no floor plan, children placed by pins, exactly like continent/region/country. `wilds` is new and sits at `site`, beside building/dungeon/store/tavern/inn: it is a natural place — a wood, a marsh, a graveyard — that **carries its own floor plan**, traced and bound the same way a dungeon's is. The same forest can legitimately be both at once: a `wilderness` pinned on the world map for the party's overland travel, and a `wilds` site for the one stretch of it they actually walk through room by room. Do not treat one as a renaming of the other.

**`grounds` moved from `site` to `interior` in the same change.** It was always meant to be "an outdoor area *inside* a site" — a glade, a grave plot, a hedge maze — but #817 filed it at `site` tier, so a DM dividing a wood had to call the wood itself a Dungeon and its glades Rooms. #886 moved it down to sit beside `room`: `room` is walled and roofed, `grounds` is the same idea without a roof, and both are now bound to *another* place's floor plan rather than carrying one of their own — `wilds` carries the plan a `grounds` sits inside.

`isInteriorType()` (`lib/locations/tiers.ts`) is the client mirror of `private.location_is_interior` in the database — the database is the authority, and this predicate exists so the UI never offers what it will refuse, the same relationship `isSiteType()` has with `private.location_can_hold_rooms`. `bindableSpaces()` now reads `isInteriorType(...) || isSiteType(...)` rather than special-casing the literal `"room"`, so `grounds` is offered as a bindable region target the same way `room` always was.

**`LOCATION_TYPE_COLORS` is a tier ramp, not a palette of kinds.** It runs cool-to-warm along that ladder (violet → blue → teal → lime → amber → rust), which reads as distance; within a tier, lightness steps by enclosure, darkest = most enclosed. `land` runs continent → region → country → wilderness, lightest last: wilderness is the least enclosed thing on the ladder. `site` steps six types — dungeon (underground, windowless) darkest, then building, then store/tavern/inn, then `wilds` lightest of all: a site with a floor plan but no roof, so it takes the ramp's lightest rung rather than a hue of its own. `interior` now steps two types instead of one: `room` darkest (walled and roofed), `grounds` lighter (open air, bound to a floor plan the way a room is). Dungeon and Wilderness no longer share a hue family; under the old ladder they did, which was the bug #810 fixed — they are not the same kind of map, whatever their footprint on the page. `grounds` and `wilds` sit one hue-step apart across the site/interior seam for the same reason, rather than sharing a family: a wood the party walks through *is* a site, a clearing traced inside another site's floor plan is one of its rooms. The type label already says what a place is; colour now says what kind of map it gets. Do not reshuffle it back into a red-for-dungeons rainbow. `LOCATION_TYPE_LABELS` declaration order is likewise ladder order, because both type dropdowns (DM and player) iterate it to build their options.

**DM and player share one colour system by construction**, not by discipline: `LOCATION_TYPE_COLORS` / `LOCATION_TYPE_LABELS` in `types/location.types.ts` are the single source, imported by `LocationMap`, `LocationHierarchyPanel`, `AtlasTreeRow`, `AtlasPlacePane`, `PlayerLocationCard`, `PlayerLocationDialog`, `PlayerLocationsView` and `AssetInsertPanel`. There is no second map anywhere — keep it that way rather than adding a player-side variant. `AtlasScaleRail` derives its tier swatches from the same record via `TIER_REPRESENTATIVE_TYPE`, so a rung can never drift from the places it stands for.
...
**Sites and rooms** (`SiteRoomsPanel.vue`, migration `20260904014714`, [#783](https://github.com/irongollem/grimoire/issues/783)). A **site-tier** place — building, dungeon, store, tavern, inn, wilds: the six types with a floor plan (redefined by [#810](https://github.com/irongollem/grimoire/issues/810), `grounds` swapped for `wilds` by [#886](https://github.com/irongollem/grimoire/issues/886)) — gets a Rooms panel (headed "Grounds" instead, on a `wilds` site — see below), gated by `isSiteType()` in `lib/locations/tiers.ts`. `STORE_LOCATION_TYPES` (store, tavern, inn) is now a strict *subset* of `site`, not a separate tier: before #810, "venue" sat alongside "site" as its own rung, so a tavern got a Store panel and a dungeon got a Rooms panel from two disjoint type sets. Under this ladder a tavern gets both — its taproom's stock and its guest rooms are not mutually exclusive — and district/wilderness lose the Rooms panel entirely, since neither has a floor plan.

Panel rows are ordinary **direct children** typed `room` or, since #886, `grounds` — the panel's `isInteriorType()` filter, not a literal `location_type === "room"`. There is deliberately no membership table: `parent_id` already owns that fact, and a second writer for it is the shape [#780](https://github.com/irongollem/grimoire/issues/780) exists to remove. A `building` nested inside a site is not a room of it — it is a child *site* with its own panel.

**Which type a new row gets follows the parent (#886).** `SiteRoomsPanel`'s inline add row creates `grounds` under a `wilds` site and `room` under every other site type — a DM dividing a wood should not end up with parts of it called rooms. The panel adapts its own empty-state copy and add-row placeholder to match ("No grounds yet — add the first one below." / "Add grounds…"), and `LocationDetailSections` swaps the section's `<h2>` between "Rooms" and "Grounds" the same way. The type stays freely editable afterward on the child's own Details form; the panel only picks a sensible default.
...
- **`edge_key`** ("14,6:W" in the map's own cell space — called `source_edge_key` until epic #884) and **`dungeon_feature_id`** (nullable FK to `dungeon_features`, `on delete set null`) arrive with the same migration. It lets a re-publish update the door it derived instead of duplicating it — and since #884 it says *where the door is* rather than who made it, so a hand-placed door carries one too; provenance moved to `derived_from` (`'dm' | 'publish'`), the same column `location_map_regions` already had. `dungeon_feature_id` closes a two-catalogues-one-fact gap: a "Secret Door" feature and a door with `is_secret` used to be the same fact recorded twice, with the perception DC retyped into `lock_note` as prose. Only three of the seven `DUNGEON_FEATURE_TYPES` — Secret Door, Hidden Passage, Moving Wall — *are* connections, so a door may *cite* a feature rather than folding features into doors outright; a door's own guard trigger checks the cited feature is the caller's own.
...
- **`grid_calibration` was already the answer, and is now shared.** It predates this work (`GridCalibrationDialog.vue` + `src/lib/battlemap/gridCalibration.ts`, built for battle maps) but was never VTT-specific in practice — the old M5 Save to Atlas wrote one on every save straight from the bake's own padding, and Publish to Atlas writes the same shape today for the Drawing's `map_layer_calibration` (`useMapPublish.ts`'s `computeBakedDimensions` call). Converging on it means a DM who traces rooms on a scan and later ticks "Battle map" is not asked to calibrate the same picture twice. It gained `origin_cell_x/y` because the bake insets the drawing by `DEFAULT_BAKE_PADDING_CELLS`, so a baked image's cell (0,0) is not the map's — without recording that offset a region traced on a bake could not be matched to the `CellMetadata` authored underneath it.

  `useMapExport.ts`'s AI-styled save **does** write one now, on both of its entry points — see cartographer.md's M8 section for the full argument, but the short version: the AI input is fitted to the provider's accepted aspect window and a TV-sized pixel budget on the client before it is sent (`src/cartographer/bake.ts`'s `fitStyleInputCanvas`), and `supabase/functions/style-map/index.ts` derives the size it requests from the provider from that same uploaded image's own dimensions, never a client-claimed field — so the request always matches what was sent. With no mismatch between what's sent and what's asked for, the resize that happens between send and result is a plain fit onto the same shape, and a `GridCalibration`'s fields are all ratios that leaves unchanged — so the calibration computed from the fitting geometry *before* sending is safe to write against whatever comes back, with no need to measure it. `styledPictureCalibration()` (`src/cartographer/aiStyleInput.ts`) builds it; `useSaveStyledSitePicture` and `useUpdateLocationPicture` carry it into the write. This is a first guess, not a guarantee — the model can still shrink or grow the grid a little in its own render, and a console warning fires if the returned image's size drifts from what was requested — which is why Calibrate stays available and opens seeded from it, same as any other Picture.
- **Three states, and no invented default — except the AI styler's own first guess, which says so out loud rather than pretending to be exact.** No layer at all (see the stack below) → no map surface at all, just a prompt. An image but no calibration → the map, read-only, with one action to calibrate. Both → the grid and tracing. A DM-uploaded scan is never guessed at: `!!grid_calibration` also gates the VTT (`EncounterRunner.vue`, `PlayerEncounterPanel.vue`), so a made-up value there would silently switch those on at the wrong scale. The AI styler's save is the one deliberate exception, and only because the geometry it guesses from is actually known rather than eyeballed (see above) — a flattened Picture reads as "Calibrated N cells wide" like any other, and Calibrate is never locked or hidden because of it.
...
- **A region binds to any addressable space, not only a room** ([#818](https://github.com/irongollem/grimoire/issues/818)). An interior space (a room, or — since #886 moved `grounds` to the `interior` tier — a `grounds` courtyard), or a *nested site*: either occupies an area of the parent's floor plan exactly as a room does, and tracing its footprint to click through into it is the same descent a pin already gives, drawn as a polygon instead of a point. The column was called `room_location_id` until #818 and was renamed rather than stretched: a name that says `room` while holding a `grounds` id is the kind of thing that gets misread later. `bindableSpaces()` in `lib/locations/tiers.ts` mirrors the guard's type half — `isInteriorType(...) || isSiteType(...)`, since #886 — so the picker never offers what the database will refuse — the database stays the authority, and a rejection still surfaces as a toast.
- **The map is a stack, and every layer can be empty (epic [#884](https://github.com/irongollem/grimoire/issues/884), wave 1, migration `20260914194936`).** Bottom up: the **Picture** (`map_url` + `grid_calibration` — a scan, a photo, or the AI styler's render), the **Drawing** (`map_layer_url` + `map_layer_calibration` — a *transparent* bake of the Cartographer drawing in `source_map_id`, so a book map shows through where nothing is painted), and the **Plan** (regions, doors, placements). A site with neither image may still hold a Plan on a blank grid, `plan_size = {cols, rows}`. `src/lib/locations/mapStack.ts` is the one reader: `buildMapStack()` picks the primary image (Drawing, else Picture, else a synthetic blank ground), `frameCalibration()` picks the calibration the plan is drawn in (two calibrations exist because two images need two), `placePicture()` maps a Picture into a Drawing's cell space, and `hasAnyMapLayer()` replaces every `!!map_url` "has a map" test — the battle map, readiness, the tree, the zoom and the reveal control all go through it. `MapStackImage.vue` renders the imagery (an intrinsic-ratio SVG for the blank ground, because a sized div squashes under the image sizing classes) and `MapFrame` measures it. Publish (`useMapPublish.ts`) writes the Drawing and never `map_url`; the styled save (`useMapExport.ts`) writes the Picture — and, from Atlas Build's own entry point (the Layers panel's Style with AI), unlinks the Drawing in the same write rather than leaving it published on top (`useSaveStyledSitePicture`; see this file's own Layers panel section and cartographer.md's M8 for why, and what shows in the meantime on a save this module doesn't make). An earlier draft of #784 had an `underlay_url` that was removed for reporting "no map yet" on a site that had one; the stack does not repeat that mistake because nothing reads a layer column directly. Players receive every layer under the one `is_map_shared` gate, pictures included — frame 16's withheld-picture rule was reversed by decision 7 of the epic ("it's a game, not a hospital or a bank"); structure is still composed from cells by `get_player_visible_site_state`.
- **There is no live Cartographer canvas here for *display*, and that is deliberate — epic #884 brings the editor into Build mode for *editing*, a different question.** #784 rendered `dungeon_maps.layers` live, over the image, and the migration header called that "the point". It was not: `map_url` and `source_map_id` are written together by Save to Atlas, so **the bake already is the render**, and drawing the tiles over their own bake shows the same picture except when the map was edited without re-baking — for which the answer is re-baking. Deleted with it: `useSiteMapRuntimes.ts`, `layersBoundingBox`, `collectUsedPackRefs`, and `renderMap`'s `transparentBackground` option — though **transparency came back in #884 as `BakeOptions.transparent`**, because the Drawing layer must let a Picture beneath it show through where nothing is painted; the PNG download and the AI-input bake stay opaque. What #789 needs from the Cartographer is cell *data* — keys joining `CellMetadata` — not a second renderer.
- **Client side** (`MapRegionsLayer.vue`, `SiteMapRegionList.vue`, `useLocationMapRegions.ts`, `src/lib/locations/gridCalibration.ts`, `src/lib/locations/siteMap.ts`, `src/types/locationMapRegion.types.ts`). `MapRegionsLayer` is mounted by `LocationMap.vue` — the one composite, alongside `MapPinsLayer` — for site-tier locations with the presence gate above satisfied; each caller (`AtlasPlacePane`, `SiteRunSurface`) fetches its own `rooms`/`regions`/`calibration` and passes them in, the same way each already passed `pins`. `SiteMapRegionList` (the "Room shapes"/"Untitled shapes" CRUD list) is mounted by `LocationMap.vue` too, in browse mode only — mirroring how the pins layer's own "Unplaced children" list already lived inside `LocationMap.vue` rather than in a caller. The image renders through `MAP_IMAGE_SIZING` — the shared constant every layer uses — so none of them can disagree about its size; `siteMap.ts` is down to `toggleCell` and `isCellOnImageGrid`.
...
- **Staleness** (`publishStaleness()` in `siteReadiness.ts`, rendered by the Layers panel's Drawing row since epic #884 — the `SiteMapSourceStrip.vue` that used to show it is deleted, because two surfaces saying the same thing is how they drift apart). `dungeon_maps.rev` (migration `20260908215643`) is a revision counter bumped by trigger whenever a map's `layers` or `metadata` change — renaming or retagging a map does not count, so those saves leave `rev` alone. `locations.map_published_rev` records which rev the site's last publish carried; null means "never published from the Cartographer" (a scanned page, a photo). `publishStaleness()` is null when there's nothing to compare (no source map, or the last publish already carries the current rev) — the fresh state the Drawing row renders — and otherwise diffs `structureFromSite()` (a minimal `DerivedStructure` rebuilt from the Atlas's *own* regions and doors' `edge_key`s) against a fresh `deriveStructure(map)` with the Cartographer's own `structureDelta`, the same diff the publish review itself uses.
...
- This namespace (`party:${locationId}`, keyed as above) stays distinct from the Atlas pane's own `ambience:${id}` (`useAmbiencePlayback.ts`), so the two producers cannot steal the slot from each other.
...
- Starting a session with the pane's own ambience playing hands the slot over immediately rather than leaving a stray scene playing; ending one resumes it.
...
- **Bulk selection (#885)** — "Select" `ListActionButton` toggles selection mode; each row wraps in `BulkSelectableCard` (`corner="top-left"`, over the emblem tile rather than the trailing reveal/chevron group). `BulkScopeBar` then offers "Move to campaign" and "Copy to campaign…", wired via `useBulkSelection` + `useMoveToCampaignFlow` + `useCopyToCampaignFlow` — the same composables and shapes `MonsterList.vue`/`NpcList.vue` use, all owned inline in `FactionListView.vue` (no separate `FactionList.vue` child component exists, unlike monsters/NPCs). `factions` carries the `enforce_quota` trigger, so a copy past quota reopens the same `showPaywall` ref the list's own "New Faction" create-gate uses. **"Make available in all campaigns" is not offered here** (`bulkScopeAllowsGeneral("factions")` is false, `useBulkCampaignScope.ts`): `faction_deities.campaign_id` is NOT NULL, so a general move could never carry a faction's deity links. A move to a specific campaign carries a faction's `faction_deities` rows along with it; the other four faction join tables (members, locations, items, inter-faction relations) carry no `campaign_id` of their own — they're scoped transitively through the faction — so nothing needs updating on them.
...
| `related_location_ids`  | uuid[]           | Non-hierarchical links to other locations (trade routes, tunnels, connected districts); shown in editor as inline chip picker and in the Atlas pane as "Related Locations" chips |
