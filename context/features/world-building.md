...
**Clickable rooms on a site's map** (`location_map_regions`, `locations.underlay_url`, migration `20260904142401`, [#784](https://github.com/irongollem/grimoire/issues/784), epic [#780](https://github.com/irongollem/grimoire/issues/780)). A site-tier place can show a map with clickable rooms, composited from three layers, any of which may be absent: an **underlay** (a static reference image — a scanned module page, a photo of a hand-drawn map), the **map** (a *live* `dungeon_maps` construct reached via `locations.source_map_id`, rendered from its `layers` — not a baked image), and **regions** (`location_map_regions` rows, each a set of grid cells bound to a `room` child location). The grid is the coordinate space; the underlay and the map are optional decoration over it — a DM can trace clickable rooms directly onto a scanned page without ever opening the Cartographer, and the cell keys still line up with a drawn map's authored cell metadata when one exists.
...
- **`underlay_url` is DM-only, deliberately distinct from `map_url`.** The two have different lifetimes (you keep the scan when you redraw the map over it), and licensing is the reason it never reaches players: `get_player_visible_locations` nulls it in the projection, same treatment as `notes` and `audio_theme` — the underlay is typically a scanned page from a book the DM owns, and personal prep use is not the same act as redistributing it to other accounts. It uploads through the existing `location-images` bucket (via `ImageUpload`), not a new one.
