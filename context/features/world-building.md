...
**Six sibling panels, deliberately not extracted.** `LocationDetailSections` now mounts `StoreInventory`, `SiteRoomsPanel`, `LocationPlacements`, `LocationDoors`, `LocationStateControls` and `SiteMapView`. They look related because they share two idioms the app already had — the bordered list row (`rounded-md border border-border bg-card px-3 py-2`) and the dashed inline-add box ending in an `AppButton` — not because this work duplicated anything. Their interaction models genuinely differ: commerce pricing, drag reorder, a kind-typed exclusive-arc picker, a directional graph edge with three authored flags, a flat row of state toggles, and a canvas.
...
**Sites and rooms** (`SiteRoomsPanel.vue`, migration `20260904014714`, [#783](https://github.com/irongollem/grimoire/issues/783)). A **site-tier** place — district, building, dungeon, wilderness — gets a Rooms panel, exactly as a venue-tier place gets a Store panel. `STORE_LOCATION_TYPES` is not "three types" but precisely the venue tier, so this is the same mechanism with one more tier, gated by `isSiteType()` in `lib/locations/tiers.ts`.
...
- **Where a room may live** is enforced by `locations_room_parent_guard`: site tier **plus venue tier**. The panel is site-only, but the constraint is wider on purpose — renting a room above a tavern is ordinary, and a check that rejects it would be hit by a real DM at a real table. Where a panel appears and what the data permits are different questions and must not share one predicate. What it does forbid is a room inside a room, a room under a world/region/continent, and a top-level room. A place that already holds rooms also cannot change into a type that cannot hold them. Cover: `supabase/tests/location_room_parent_guard.test.sql`.
...
3. #797 later wants to know which quest a fact was asserted during. On a log that is `alter table add column`; on a boolean it is a backfill that cannot be done, because the information was never kept.
...
**Clickable rooms on a site's map** (`location_map_regions`, migration `20260904142401`, [#784](https://github.com/irongollem/grimoire/issues/784), epic [#780](https://github.com/irongollem/grimoire/issues/780)). A site-tier place can show a map with clickable rooms, composited from three layers, any of which may be absent: the **image** (`locations.map_url` — a Cartographer bake, an uploaded scan, a photo of a hand-drawn page), the **live map** (a `dungeon_maps` construct reached via `locations.source_map_id`, rendered from its `layers` — not a baked image), and **regions** (`location_map_regions` rows, each a set of grid cells bound to a `room` child location). A DM can trace clickable rooms directly onto a scanned page without ever opening the Cartographer, and the cell keys still line up with a drawn map's authored cell metadata when one exists.

- **The binding is relational; the geometry is not.** `room_location_id` is a real FK (cascade), `cells` is a jsonb array of `CellKey` strings — the same split `location_placements` and `location_doors` make, and deliberately not the shape `metadata.room_ids` left behind. `site_location_id` is required even on an unbound region, because a DM traces shapes off the page first and names them second; `room_location_id` null means exactly "traced but not yet named", not broken. A partial unique index allows only one *bound* region per room while several unbound ones coexist; a trigger (`guard_location_map_region_room`) rejects binding to a non-`room` or to a room of a different site. The client does not re-derive either rule — a rejection surfaces as a toast.
- **There is no second image column, and an earlier draft's `underlay_url` was removed before it shipped.** `map_url` already *is* the picture of a place whatever its provenance, and `is_map_shared` already decides whether players see it. Two columns meant this panel reported "no map yet" on a site that plainly had one, because it consulted only its own field — the same two-answers-to-one-question shape the epic exists to remove. Regions overlay `map_url`; the location editor owns that field, and this panel has no uploader of its own.
- **Client side** (`SiteMapView.vue`, `useLocationMapRegions.ts`, `useSiteMapRuntimes.ts`, `src/lib/locations/siteMap.ts`, `src/types/locationMapRegion.types.ts`). Mounted in `LocationDetailSections.vue` for site-tier locations (`isSiteType`), above the Rooms panel. `renderMap()` in `src/cartographer/renderMap.ts` — the same pure rendering engine `CartographerEditorView` and the map-export pipeline use — is reused **read-only** here as a third consumer; this is not a merge of the authoring tool with the encounter runner (CLAUDE.md forbids that), it is a third caller of a pure function that takes plain data and a canvas context, nothing more.
  - **`renderMap` gained an optional transparent ground**, and that is the one change made to the engine rather than around it. It used to fill its whole canvas with the editor's dark ground unconditionally, which is the correct reading of "nothing here yet" *in the editor* but hides the picture underneath everywhere the map has not painted — most of the page while a DM is still tracing. `MapRenderScene.transparentBackground` defaults to `false`, so the Cartographer editor and the export pipeline are untouched; the Atlas passes `true`. Compositing is still DOM layering rather than alpha blending — the image is a full-size `<img>` and the map renders into its own canvas sized to its painted bounding box (via `layersBoundingBox`, which scans all four layers, since a wall can exist on a cell with no floor tile) — but unpainted space inside that box now lets the page through, which is what "the drawn map overlays the book image" actually means.
  - **The addressable grid** (`resolveGridBounds`) is the union of the live map's painted extent and every region's traced cells, padded by one cell, falling back to a fixed default extent when neither exists yet — a site with only a picture still needs a grid to trace onto. **This is known to be wrong and is being replaced** ([#805](https://github.com/irongollem/grimoire/issues/805)): deriving the extent from the regions means tracing a shape resizes the coordinate space the shapes were traced in, so the picture is re-stretched under them and earlier shapes stop lining up with the features they were traced over.
  - **Tile packs load read-only** (`useSiteMapRuntimes.ts`), scoped to only the `(pack_id, pack_version)` pairs a map's cells actually reference (`collectUsedPackRefs`) rather than eagerly loading every bundled pack the way the editor does — a viewer only ever needs to render what is already on the map. Bundled-pack manifest URLs are derived from the naming convention (`/cartographer/<pack_id>/v<pack_version>/manifest.json`) rather than duplicating the editor's own hard-coded pack list, which is local to that view and not exported; custom packs route through the existing `useTilePacks`/`loadUserPack`.
  - **Interaction**: with no region selected, clicking a bound region's cell navigates to that room; clicking an unbound region's cell selects it for naming/tracing. With a region selected, clicking any grid cell toggles it in or out of that region (`toggleCell`) — deliberately just a grid, not a freeform polygon tool. Rooms with no region yet are still listed (not hidden), so a site is usable before it is fully traced.
...
**The site runner** (`SiteRunSurface.vue`, `lib/locations/siteRun.ts`, [#791](https://github.com/irongollem/grimoire/issues/791), epic #780, Phase 1). One surface to run a dungeon at the table with **no quest open at all** — everything it needs already existed from #783–#790; this phase composes it. Three zones: **the place** (the map, in a new `mode="run"`, plus a plain click-to-move room list — the list is what makes a site runnable before any of it is traced, since the map only mounts once something is); **the current room** (description, `LocationStateControls`, `LocationPlacements`, `LocationDoors`, all composed inline — the DM never navigates away to reach any of it); **context** (rooms explored, doors still locked, at a glance).
...
- **`SiteMapView` gained a `mode` prop (`"browse"` default, `"run"`) rather than a fork.** Browse mode is unchanged. Run mode swaps click behaviour (a bound region moves the party instead of navigating to it), swaps the region palette (party-here blue, reachable green, unreachable stone-grey, untraced nearly invisible — repurposing browse mode's three tracing colours rather than adding a fourth), and hides the editing-only sections (reference-image upload, the Rooms list, Untitled shapes) — a DM at the table shouldn't be prompted to trace.
