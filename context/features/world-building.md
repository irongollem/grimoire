...
- **Three states, and no invented default.** No layer at all (see the stack below) → no map surface at all, just a prompt. An image but no calibration → the map, read-only, with one action to calibrate. Both → the grid and tracing. A guessed calibration is deliberately *not* seeded: `!!grid_calibration` also gates the VTT (`EncounterRunner.vue`, `PlayerEncounterPanel.vue`), so a made-up value would silently switch those on at the wrong scale.
