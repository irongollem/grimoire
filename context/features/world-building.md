...
**`LocationDetailSections.vue` — shared by the sheet and the pane.** Holds the eight sections that are *about* a place rather than *where it sits*: Description, Related Locations, Store, Rooms, Prepared Here, People in the Area, Encounters Here, Currently Here. Two of those are always present rather than gated on content, so the body itself is no longer conditional; the pane asks `hasSubstance` instead when it needs to know whether a place is genuinely empty. Self-contained (does its own queries; TanStack dedupes against the caller's). Each caller keeps what genuinely differs — breadcrumb, identity, sub-locations, map placement, and the Atlas's scale rail and tier groups.
