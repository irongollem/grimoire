...
**Clickable rooms on a site's map** (`location_map_regions`, migration `20260904142401`, [#784](https://github.com/irongollem/grimoire/issues/784) and [#805](https://github.com/irongollem/grimoire/issues/805), epic [#780](https://github.com/irongollem/grimoire/issues/780)). A site-tier place can show its map with clickable rooms traced onto it: the **image** (`locations.map_url` — a Cartographer bake, an uploaded scan, a photo of a hand-drawn page) with **regions** over it (`location_map_regions` rows, each a set of grid cells bound to a `room` child location). A DM can trace clickable rooms straight onto a scanned page without ever opening the Cartographer.
...
- **Client side** (`SiteMapView.vue`, `SiteMapRegionList.vue`, `useLocationMapRegions.ts`, `src/lib/gridCalibration.ts`, `src/lib/locations/siteMap.ts`, `src/types/locationMapRegion.types.ts`). Mounted in `LocationDetailSections.vue` for site-tier locations (`isSiteType`), above the Rooms panel. The image renders through `MAP_IMAGE_SIZING` — the shared constant `LocationMap` uses — so the two cannot disagree about its size; `siteMap.ts` is down to `toggleCell` and `isCellOnImageGrid`.
