    ...
    <template v-else>
      ...
      <div v-if="showForm" class="rounded-md border border-border bg-muted p-3 flex flex-col gap-3">
        ...
        <div class="flex rounded-md border border-border overflow-hidden">
          <button
            type="button"
            class="flex-1 py-1 font-cinzel text-xs font-semibold tracking-wider transition-colors"
        ...
        <div v-if="dateType === 'regular'" class="grid grid-cols-3 gap-2">
          <div>
            ...
            <input
              ...
              type="number"
              min="1"
              class="w-full bg-card border border-border rounded-md px-2 py-1.5 font-fell text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
            />
          </div>
          <div>
        ...
        <div v-else class="grid grid-cols-2 gap-2">
          ...
          <div>
            ...
            <select
              ...
              class="w-full bg-card border border-border rounded-md px-2 py-1.5 font-fell text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
            >
              <option v-for="f in adapter.intercalaryDays" :key="f.name" :value="f.name">
                {{ f.name }}
              </option>
            </select>
          </div>
        ...
        <div class="flex items-center gap-2 justify-end">
          ...
          <button
            type="button"
            :disabled="creating"
            class="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 font-cinzel text-xs font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-50 transition-opacity"
            @click="pinDate"
          >
