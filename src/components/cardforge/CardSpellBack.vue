  ...
  <div class="card-shell" :style="{ '--fc': frameColor }">
    <div class="card-face">
      ...
      <div class="card-header">
        <span class="card-name">{{ truncateCard(data.name, 20) }}</span>
        <span class="back-label">Spell</span>
      </div>

      <!-- Description -->
      <div class="desc-block">
        <p class="desc-text">{{ truncateCard(data.description, 220) }}</p>
      </div>
      ...
      <template v-if="data.higher_levels">
        ...
        <div class="higher-block">
          <p class="higher-text">{{ truncateCard(data.higher_levels, 120) }}</p>
        </div>
      </template>
      ...
      <template v-if="data.material">
        ...
        <div class="higher-block">
          <p class="higher-text">{{ truncateCard(data.material, 80) }}</p>
        </div>
      </template>
...
.desc-block { padding: 4px 6px; flex: 1; overflow: hidden; }
.desc-text { font-size: 5.5px; color: #2a2018; line-height: 1.5; margin: 0; }
.higher-block { padding: 2px 6px 3px; }
.higher-text { font-size: 5.5px; color: #3a3028; font-style: italic; margin: 0; }
.flavor-footer {
