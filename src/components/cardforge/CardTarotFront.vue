<template>
  <div class="card-shell" :style="{ '--fc': frameColor }">
    <div class="card-face">
      ...
      <!-- Ability scores (more readable at tarot size) -->
      ...
      <!-- First special ability / trait preview -->
      <div v-if="firstAbility" class="ability-preview">
        <span class="ab-name">{{ firstAbility.name }}.</span>
        <span class="ab-desc">{{ truncate(firstAbility.description, 130) }}</span>
      </div>
...
<style scoped>
/* Tarot card: 70mm × 120mm. Screen preview: 245px × 420px */
.card-shell {
  width: 245px;
  height: 420px;
  border-radius: 12px;
