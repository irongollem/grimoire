...
const statsRows = computed((): StatRow[] => {
  ...
  if ("saving_throws" in sb && sb.saving_throws)
    rows.push({ label: "Saving Throws", value: sb.saving_throws });
  ...
  if ("damage_vulnerabilities" in sb && sb.damage_vulnerabilities)
    rows.push({ label: "Vulnerabilities", value: sb.damage_vulnerabilities });
...
const flavorSection = computed((): FlavorSection | null => {
  if (!props.data.notes && !props.data.habitat) return null;
  return {
    heading: "Notes",
    text: props.data.notes ?? "",
    location: props.data.habitat,
  };
});
</script>
