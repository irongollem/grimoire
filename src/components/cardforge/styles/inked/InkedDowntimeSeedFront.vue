<template>
  <InkedShell :tarot :accent="accentForDowntimeSeed(data)">
    <div class="ik-art">
      ...
      <!-- Note-yielding seeds carry no art (`notes` has no image column), so the
           archetype's own glyph face stands in — the same fallback the app uses. -->
      <div v-else class="ik-art-ph" :style="{ '--glyph-bg': accent }">
        <span class="ik-art-glyph">{{ glyph }}</span>
        <span class="ik-art-label">{{ rewardNoun }}</span>
      </div>
      <div class="ik-scrim" />
    </div>
    <div class="ik-top">
    ...
    <div class="ik-bottom">
      ...
      <div class="ik-sub">{{ rewardName }}</div>
      <div class="ik-stats">
...
.ik-name {
  font-family: "UnifrakturCook", "Cinzel", serif; font-size: 17px; font-weight: 700;
...
.ik-stat {
  ...
  font-family: "Cinzel", serif; font-size: 8px; font-weight: 900; color: #fff;
