<template>
  <div class="ik-shell" :class="{ tarot }" :style="{ '--fc': frameColor }">
    <div class="ik-art">
      <FocalImage v-if="data.image_url" :src="data.image_url" format="portrait" :focal-point="data.portrait_focal_point" print />
      <div v-else class="ik-art-ph">
        <span class="ik-art-glyph">{{ data.monster_type.charAt(0).toUpperCase() }}</span>
        <span class="ik-art-label">{{ data.monster_type }}</span>
      </div>
      <div class="ik-scrim" />
    </div>
    <div class="ik-top">
...
import { MONSTER_COLORS } from "@/types/card.types";
const props = defineProps<{ data: Monster; tarot?: boolean }>();
const frameColor = computed(() => MONSTER_COLORS[props.data.monster_type] ?? "#5a0e0e");
const badge = computed(() => {
  const cr = props.data.stat_block?.challenge_rating;
  return (cr !== null && cr !== undefined) ? "CR " + cr : null;
});
const typeTag  = computed(() => "MONSTER · " + props.data.monster_type.toUpperCase());
const typeLine = computed(() => props.data.size + " " + props.data.monster_type + " · " + props.data.alignment);
const hp  = computed(() => props.data.stat_block?.hit_points?.split(" ")[0] ?? "—");
const ac  = computed(() => String(props.data.stat_block?.armor_class ?? "—"));
const spd = computed(() => (props.data.stat_block?.speed ?? "—").replace(/ ft\.?/g, "'").slice(0, 6));
const stats = computed(() => [{ label:"HP",value:hp.value },{ label:"AC",value:ac.value },{ label:"SPD",value:spd.value }]);
</script>
<style scoped>
.ik-shell {
