<template>
  <ModernShell :tarot face="front" :accent="accentForDowntime(data)">
    <div class="md-art">
      <FocalImage v-if="portrait" :src="portrait" format="portrait" print />
      <!-- The deck has no artwork yet, so the glyph face is the norm, not a stopgap. -->
      <div v-else class="md-art-ph" :style="{ '--glyph-bg': data.accent }">
        <span class="md-art-glyph">{{ glyph }}</span>
        <span class="md-art-label">{{ riskLabel }}</span>
      </div>
    </div>
    <div class="md-scrim" />
