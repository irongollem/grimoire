<template>
  <div class="flex flex-col h-full min-h-0">
        ...
        >
          ...
          <div
            ...
            :class="msg.user_id === myUserId
              ...
              : 'bg-muted/60 border border-border'"
          >
            <div v-if="msg.user_id !== myUserId || msg.recipient_user_id" class="flex items-center gap-1 mb-0.5">
              <p class="font-cinzel text-[10px] font-semibold tracking-wider text-primary">
                {{ msg.user_id === myUserId ? 'You' : msg.sender_name }}
              </p>
              <span v-if="msg.recipient_user_id" class="font-fell text-[10px] text-amber-400 italic">
    ...
    <Transition name="dice-expand">
      <div v-if="diceOpen" class="shrink-0 border-t border-border bg-muted/20 px-3 py-2 space-y-2">
    ...
    <div v-if="members && members.length > 1" class="shrink-0 px-2 pt-1.5 flex items-center gap-2">
      <span class="font-cinzel text-[10px] text-muted-foreground tracking-wider shrink-0">To:</span>
      <select
    ...
    <div class="shrink-0 border-t border-border bg-card px-2 py-2 flex items-end gap-1.5">
      ...
      <textarea
        ...
        v-model="inputText"
        rows="1"
        :placeholder="whisperTarget ? `Whisper to ${whisperTargetName}…` : 'Type a message…'"
...
function recipientName(userId: string): string {
  ...
  return m?.display_name ?? "Player";
}

// ── Whisper target ─────────────────────────────────────────────────────────────
const whisperTarget = ref<string>("");
const whisperTargetName = computed(() => recipientName(whisperTarget.value));
...
const totalDice = computed(() => ALL_DICE.reduce((s, d) => s + (diceCounts[d] ?? 0), 0));

function toggleDie(d: DieSize) { diceCounts[d] = (diceCounts[d] ?? 0) > 0 ? 0 : 1; }
