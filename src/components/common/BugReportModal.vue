      ...
      <div class="px-5 py-4 space-y-4 flex-1">
        ...
        <div class="space-y-1.5">
          ...
          >
            ...
            />
            <!-- `rounded-full bg-card/90` overrides the variant's square, fully
                 opaque box: this sits *on* the screenshot, so it needs to read as
                 a chip over the image rather than a control beside it. -->
            <AppButton
              variant="ghost"
              size="icon-2xs"
              :icon="IconClose"
              class="absolute top-1.5 right-1.5 rounded-full border border-border bg-card/90"
...
// Runs on the shell's `afterLeave`, once the panel has actually finished
// animating out — rather than after a guessed `setTimeout` delay.
function reset() {
  kind.value = "bug";
  form.value = {
    where: "",
