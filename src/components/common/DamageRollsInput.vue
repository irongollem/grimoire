<template>
  <div class="flex flex-col gap-2">
    ...
    <div class="flex gap-2 items-center mt-1">
      <input
        v-model="parseInput"
        placeholder="Paste expression: 2d6 fire + 1d6 slashing…"
...
const props = defineProps<{ modelValue: DamageRoll[] }>()
...
const totalAvg = computed(() =>
  props.modelValue.reduce((sum, r) => sum + parseDiceAvg(r.dice), 0)
)

function updateRow(i: number, field: 'dice' | 'type', value: string) {
  const updated = props.modelValue.map((r, idx) =>
