    ...
    <Transition name="dice-drop">
      ...
      >
        <button
          ...
          :class="i === activeIdx ? 'bg-primary/20 text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'"
          @mousedown.prevent="select(s)"
        >
          <span class="text-primary font-semibold">{{ s.slice(0, activeWord?.word.length) }}</span>{{ s.slice(activeWord?.word.length) }}
        </button>
      </div>
    </Transition>
  </div>
...
interface WordMatch { word: string; start: number; end: number }
...
function refreshSuggestions() {
  ...
  if (match) {
    suggestions.value = (DAMAGE_TYPES as readonly string[]).filter(t =>
      t.startsWith(match.word) && t !== match.word // hide exact match
    )
  } else {
    suggestions.value = []
  }
...
<style scoped>
.dice-drop-enter-active { transition: opacity 0.1s ease, transform 0.1s ease; }
.dice-drop-leave-active { transition: opacity 0.08s ease; }
.dice-drop-enter-from  { opacity: 0; transform: translateY(-4px); }
.dice-drop-leave-to    { opacity: 0; }
</style>
