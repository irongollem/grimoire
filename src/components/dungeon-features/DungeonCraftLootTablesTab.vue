    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
      ...
      >
        <div class="flex items-start justify-between gap-2 mb-1">
          ...
          <span v-if="t.cr_tier !== 'any'" class="font-cinzel text-2xs px-1.5 py-0.5 rounded bg-primary/10 text-primary font-semibold tracking-wider shrink-0">{{ LOOT_CR_TIER_LABELS[t.cr_tier] }}</span>
        </div>
