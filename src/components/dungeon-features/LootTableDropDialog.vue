<template>
  <AppModal :open="open" size="md" @close="emit('close')">
    <ModalHeader title="Drop chest in chat" :icon="IconPackageOpen" tone="primary" closeable @close="emit('close')" />

    <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4 flex flex-col gap-3">
      <!-- How it works -->
    ...
    <div class="px-5 py-4 border-t border-border flex items-center justify-end gap-2 shrink-0">
