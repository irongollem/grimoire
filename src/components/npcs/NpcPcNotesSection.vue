<template>
  <div class="space-y-2">
    ...
    >
      ...
      <div v-if="editingId !== note.id" class="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            type="button"
            class="font-cinzel text-[10px] text-muted-foreground hover:text-destructive tracking-wider transition-colors"
        ...
        <RichTextEditor v-model="editText" placeholder="How do they know each other…" min-height="80px">
          <template #toolbar-end>
            <div class="ml-auto flex items-center gap-2 pl-1">
              ...
              <button
                type="button"
                class="px-2 h-6.5 font-cinzel text-[10px] tracking-wider rounded text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
                @click="cancelEdit"
              >Cancel</button>
            </div>
          </template>
    ...
    <button
      ...
      type="button"
      class="inline-flex items-center gap-1 px-2 py-1 font-cinzel text-[10px] font-semibold tracking-wider border border-border rounded-md hover:bg-muted transition-colors text-muted-foreground hover:text-foreground"
      @click="showForm = true"
    >
      + Add
    </button>
