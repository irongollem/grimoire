    ...
    >
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-baseline gap-2 flex-wrap">
          ...
          <span class="font-cinzel text-[10px] tracking-wider shrink-0" :style="{ color }">{{ categoryLabel }}</span>
        </div>
        <div v-if="preview" class="font-fell text-xs text-muted-foreground mt-0.5 line-clamp-1">{{ preview }}</div>
        <div class="flex flex-wrap items-center gap-3 mt-1.5">
          <span class="font-cinzel text-[10px] text-muted-foreground/60 tracking-wider">{{ date }}</span>
          <slot name="meta" />
        </div>
      </div>
      <ChevronDown
