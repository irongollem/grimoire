        ...
        <div class="relative w-full max-w-sm rounded-xl border border-border bg-card shadow-2xl">
          ...
          <div class="px-5 py-4 space-y-4">
            ...
            <div class="space-y-1">
              <div class="flex items-baseline justify-between">
                <span class="font-cinzel text-xs text-muted-foreground tracking-wider">HP</span>
                <span class="font-cinzel text-sm font-bold" :class="hpColor">
                  {{ previewHp }} <span class="font-fell text-xs text-muted-foreground font-normal">/ {{ member.max_hp }}</span>
                </span>
              </div>
              <div class="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                <div class="h-full rounded-full transition-all" :class="hpBarColor" :style="{ width: `${previewHpPct}%` }" />
              </div>
            </div>
            ...
            <div class="rounded-lg border border-border bg-muted/20 p-3 space-y-2">
              <div class="flex items-center justify-between">
                <span class="font-cinzel text-xs font-semibold text-muted-foreground tracking-wider">HIT DICE</span>
              ...
              <template v-if="mode === 'short'">
                ...
                <div v-if="rolls.length" class="flex flex-wrap gap-1">
                  ...
                  >+{{ roll }}</span>
                  <span class="font-cinzel text-[10px] px-1.5 py-0.5 rounded bg-elven-green/10 text-elven-green/80 border border-elven-green/20">
              ...
              <template v-else>
                <p class="font-fell text-xs text-muted-foreground italic">
                  You regain {{ diceRecovered }} expended hit {{ diceRecovered === 1 ? "die" : "dice" }} after sleeping.
                </p>
              </template>
            </div>

            <!-- Long rest summary -->
            <div v-if="mode === 'long'" class="space-y-1 text-xs font-fell text-muted-foreground">
              <p class="flex items-center gap-1.5"><span class="text-elven-green">✓</span> Full HP restored</p>
              <p v-if="hasShortRestResources || hasLongRestResources" class="flex items-center gap-1.5"><span class="text-elven-green">✓</span> All class resources restored</p>
              <p v-if="hasSpellSlots" class="flex items-center gap-1.5"><span class="text-elven-green">✓</span> All spell slots restored</p>
              <p class="flex items-center gap-1.5"><span class="text-elven-green">✓</span> Death saves cleared</p>
            </div>
          ...
          <div class="flex justify-end gap-2 px-5 pb-5">
            ...
            <button
              ...
              @click="confirm"
            >{{ mode === "short" ? "Finish Rest" : "Sleep" }}</button>
          </div>
        </div>
      </div>
    </Transition>
...
const remainingAfterSpend = computed(() => hitDiceRemaining.value - diceSpent.value);
...
const previewHp = computed(() => {
  ...
  return Math.min(props.member.max_hp, props.member.current_hp + totalHealing.value);
});

const previewHpPct = computed(() =>
  props.member.max_hp === 0 ? 0 : Math.min(100, (previewHp.value / props.member.max_hp) * 100),
);
...
const hasShortRestResources = computed(() =>
  Object.values(props.member.class_resources ?? {}).some(r => r.rest === "short"),
);
const hasLongRestResources = computed(() =>
  Object.values(props.member.class_resources ?? {}).some(r => r.rest === "long"),
);
...
function confirm() {
  ...
  if (props.mode === "short") {
    ...
    if (totalHealing.value > 0) {
      update.current_hp = Math.min(props.member.max_hp, props.member.current_hp + totalHealing.value);
    }
    // Hit dice spent
    ...
    for (const key of Object.keys(updatedResources)) {
      if (updatedResources[key].rest === "short") {
        updatedResources[key] = { ...updatedResources[key], current: updatedResources[key].max };
      }
    }
    if (Object.keys(updatedResources).length) update.class_resources = updatedResources;

    // Warlock pact slot recovery
    if (getSlotRecovery(props.member.class) === "short") {
      update.spell_slots = props.effectiveSpellSlots.map(s => ({ ...s, used: 0 }));
    }
  } else {
