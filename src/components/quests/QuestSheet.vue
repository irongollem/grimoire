<template>
  <div class="flex flex-col gap-5">
    ...
    <div class="flex flex-wrap items-center justify-end gap-2">
      ...
      <AppButton
        :icon="IconNetwork"
        :label="quest.flow_enabled_at ? 'Build flow' : 'Create flow'"
        variant="subtle"
        @click="openBuild"
      />
      <AppButton
      ...
      />
      <AppButton
        v-if="quest.flow_enabled_at"
        label="Return to legacy"
        variant="subtle"
        :disabled="rollbackMutation.isPending.value"
        @click="rollbackConversion"
      />
      <AppButton
        :icon="IconDelete"
        label="Delete"
    ...
    <QuestFlowConversionPanel
      v-if="conversionOpen"
      :quest="quest"
      @cancel="conversionOpen = false"
      @converted="openConvertedFlow"
    />
    <p v-if="rollbackError" role="alert" class="text-caption text-destructive">{{ rollbackError }}</p>
...
import { useRollbackQuestFlowConversion } from "@/composables/useQuestFlow";
...
const rollbackMutation = useRollbackQuestFlowConversion();

function openBuild() {
  if (props.quest.flow_enabled_at) void router.push({ query: { mode: "build" } });
  else conversionOpen.value = true;
}

function openConvertedFlow() {
  conversionOpen.value = false;
  void router.push({ query: { mode: "build" } });
}

async function rollbackConversion() {
  if (!(await confirm("Remove only the beats generated by legacy conversion and return this quest to the legacy sheet? Hand-authored beats and all legacy content remain stored."))) return;
  rollbackError.value = "";
  try {
    await rollbackMutation.mutateAsync(props.quest.id);
  } catch (cause) {
    rollbackError.value = cause instanceof Error ? cause.message : "Conversion rollback failed";
  }
}
