...
/**
 * AI-use and likeness consent-notice versions (EU AI Act Art 50(1) — see
 * context/compliance/provenance-architecture.md §3). Canonical source is
 * `supabase/functions/_shared/provenance/consent.ts`, re-exported here via
 * the `@edge-shared` alias so the client and the edge-function backstop
 * (`forge-mini`, `generate-chronicle-image`) can never drift on version.
 * Bump the canonical constants to re-prompt everyone.
 */
export { AI_USE_NOTICE_VERSION, AI_LIKENESS_NOTICE_VERSION } from "@edge-shared/provenance/consent.ts";
