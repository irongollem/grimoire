...
export interface QuestFlowNodeData { beatId: string; title: string; kind: string; visibility: string }
export type QuestFlowNode = Node<QuestFlowNodeData>;
export type QuestFlowEdge = Edge<{ edgeId: string }>;
...
export function toQuestFlowGraph(beats: QuestBeat[], edges: QuestBeatEdge[]) {
  const nodes: QuestFlowNode[] = beats.map((beat) => ({
    id: beat.id,
    type: "questBeat",
    position: { x: beat.canvas_x, y: beat.canvas_y },
    data: { beatId: beat.id, title: beat.title, kind: beat.kind, visibility: beat.visibility },
  }));
  const flowEdges: QuestFlowEdge[] = edges.map((edge) => ({
    ...
    type: "questRoute",
    class: "quest-flow-route",
    data: { edgeId: edge.id },
  }));
  return { nodes, edges: flowEdges };
}

export function moveBeatCommand(node: Pick<QuestFlowNode, "id" | "position">): QuestGraphCommand {
  return { type: "move", beatId: node.id, x: node.position.x, y: node.position.y };
}
...
export function applyFlowPositions(beats: QuestBeat[], nodes: Array<Pick<QuestFlowNode, "id" | "position">>): QuestBeat[] {
