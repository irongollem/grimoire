      ...
      <div class="flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
        ...
        <div v-if="activeCombatant" class="ml-4 flex items-center gap-2">
          ...
          <span class="font-cinzel text-sm font-bold text-foreground">
            {{ activeCombatant.type === 'monster' && (activeCombatant.reveal_state ?? 'hidden') === 'hidden' ? '???' : activeCombatant.name }}
          </span>
        </div>
      </div>
      ...
      <div class="rounded-lg border border-border bg-card overflow-hidden">
        <template v-for="combatant in visibleCombatants" :key="combatant.instance_id">
          ...
          >
            ...
            <div class="portrait-cell">
              <div class="portrait-inner" :class="isActive(combatant) ? 'portrait-active' : ''">
                ...
                <div
                  ...
                  :style="{ backgroundColor: combatant.type === 'player' ? 'rgba(99,102,241,0.2)' : 'rgba(120,53,15,0.2)', color: combatant.type === 'player' ? '#818cf8' : '#b45309' }"
                >
            ...
            <!-- Rest of row content with vertical padding -->
            <div class="row-content">
              <!-- Initiative -->
              <div class="shrink-0 w-8 text-center self-center">
                <span class="font-cinzel text-sm font-bold" :class="isActive(combatant) ? 'text-primary' : 'text-muted-foreground'">
...
// Exclude hidden monsters; unseen and revealed pass through
