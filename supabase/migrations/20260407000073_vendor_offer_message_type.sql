-- Extend type check to include vendor_offer
alter table public.campaign_messages
  drop constraint campaign_messages_type_check;

alter table public.campaign_messages
  add constraint campaign_messages_type_check
  check (type in ('chat', 'roll', 'system', 'item_drop', 'currency_drop', 'vendor_offer'));

-- Extend claim UPDATE policy to cover vendor_offer as well
...
create policy "campaign_messages_claim" on public.campaign_messages
  for update using (
    type in ('item_drop', 'currency_drop', 'vendor_offer')
