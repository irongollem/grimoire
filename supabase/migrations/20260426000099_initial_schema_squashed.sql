...
-- The storage schema (tables, types, functions, grants, RLS-enable) is created by
-- the Supabase storage service, NOT this migration: recreating it fails on a local
-- `supabase start` where the migration role cannot CREATE in / take ownership of
-- the service-owned storage schema. Only the app-specific CREATE POLICY statements
-- belong here; the storage service already enables RLS on storage.objects.
