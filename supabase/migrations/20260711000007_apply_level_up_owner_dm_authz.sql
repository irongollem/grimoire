-- Migration: apply_level_up_owner_dm_authz
-- apply_level_up (20260710000002/000003) authorized only on `user_id = auth.uid()`
-- (the row CREATOR). But assume_character deep-copies a character preserving the
-- DM's user_id and setting owner_user_id = player, and DM-built characters have
-- user_id = DM — so the level-up wizard's Confirm raised 42501 ("not authorized")
-- for every assumed character (the player isn't the creator) and for a DM leveling
-- a player-created character. The party_members UPDATE RLS already allows
-- owner_user_id or campaign DM, so all other sheet edits worked — only this RPC's
-- stricter guard diverged.
--
-- Widen the guard to mirror that RLS: creator OR owner OR campaign DM. Everything
-- else (search_path = '', the full body) is unchanged from 20260710000003.
