-- The import lands one web, not a pile. Teach the extractor the links.
--
-- A pasted page is a complete set of content — the maintainer: "every resource
-- we create should be good, and properly linked to the other resources made
-- here." The importer could only link what the prompt asked for: an NPC's
-- faction, a location's parent, a quest's giver and location, an encounter's
-- room and combatants. Everything else the schema can hold — where an NPC is
-- found (npcs.location_id), who owns a place (locations.npc_owner_id), where a
-- faction operates (faction_locations), where a scene happens
-- (quest_beats.staged_at_location_id) and what is in it
-- (quest_beat_attachments) — arrived as prose, and the DM rebuilt it by hand.
--
-- The contract gained those fields in the same change (documentImport.types.ts,
-- extractionSchema.ts); this is the half a type cannot do. Rewritten in full,
-- not appended, per 20260907000546's own rationale: an appended instruction
-- leaves the old, shorter cross-reference list sitting above the new one.
--
-- Checked against production before writing: the live row was byte-identical
-- to 20260907000546's body (length 12275, md5 c05d202e…), so this rewrites
-- what is actually running, not a drifted copy.
...
- `item_names` — items found, given or needed in the scene.
...
- a location's `parent_name`, and `owner_npc_name` — who runs or owns it (an
  innkeeper's inn, a baron's keep)
...
-- Same guard shape as 20260907000546: assert on a string only the new prompt
-- has, so a `where` that matched nothing fails the deploy instead of leaving
-- the extractor on the old contract.
do $guard$
begin
  if not exists (
    select 1 from public.ai_system_prompts
     where generator_type = 'document_import'
       and content like '%Wiring a beat to the rest of the page%'
  ) then
