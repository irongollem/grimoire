-- A keyed dungeon is a site, and its keyed areas are rooms. Say so.
--
-- A real import on 18 Sep 2026 (a gem mine, rooms M1–M13) returned the mine as
-- `"location_type": "mine"` and each area as `"mine room"`. Nothing
-- constrained the field, the importer mapped both to `other`, and the database
-- then refused every room: `guard_location_room_parent` only lets a `room` sit
-- inside one of the six site types (building, dungeon, store, tavern, inn,
-- wilds), so thirteen inserts failed and the mine arrived with no rooms and no
-- site to run.
--
-- The same run also left every beat unlinked, and one beat ("Explore the
-- Mine") without its site. Most of that was a deploy race (the page was read
-- 1.5 minutes before 20260918144110 reached production), but the prompt also
-- never said WHICH place a crawl beat stages at. The design does
-- (context/features/quests.md, quoting the Sites sheet): a beat stages at the
-- site; "a dungeon needs no beats inside it... rooms are places, not events";
-- a room's fights live in the room (encounters.location_id) and its loot in
-- the room (loot_placements.location_id). So a crawl beat names the site, not
-- a room, and room fights and room loot stay off the beat.
--
-- Two halves, shipped together: `extractionSchema.ts` now constrains
-- `location_type` to the enum (the model can no longer answer "mine"), and this
-- teaches it which member to pick: the container is a site type, the keyed
-- areas are `room`/`grounds`. A schema enum alone would stop the invention
-- but not the choice, and `other` is exactly as fatal to the rooms as "mine".
--
-- Rewritten in full from 20260918144110's body, which was verified
-- byte-identical to production before writing (length 13852, md5 ea37dee9…).
...
- `beats` — the scenes, in the order the source presents them. Each beat has a
  ...
  boxed text if it has any. A beat also records **what is in it** — see
...
begin
  if not exists (
    select 1 from public.ai_system_prompts
     where generator_type = 'document_import'
       and content like '%Location types come from a fixed list%'
  ) then
