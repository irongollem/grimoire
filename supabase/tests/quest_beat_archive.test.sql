...
select has_function('public', 'archive_quest_beat', array['uuid', 'jsonb'], 'beat archival has one transactional RPC');
select ok((select prosecdef from pg_proc where oid = 'public.archive_quest_beat(uuid,jsonb)'::regprocedure), 'archive owns runtime writes behind an explicit DM guard');
select ok(not has_function_privilege('anon', 'public.archive_quest_beat(uuid,jsonb)', 'EXECUTE'), 'anonymous callers cannot archive beats');
select ok(has_function_privilege('authenticated', 'public.archive_quest_beat(uuid,jsonb)', 'EXECUTE'), 'authenticated DMs can call the archive RPC');
...
values ('68300000-0000-4000-8000-000000000020', '68300000-0000-4000-8000-000000000010', '68300000-0000-4000-8000-000000000030', '68300000-0000-4000-8000-000000000031');
