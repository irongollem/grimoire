...
select has_function('public', 'create_quest_beat_with_route', array['uuid', 'text', 'text', 'double precision', 'double precision', 'uuid'], 'beat and incoming route share one RPC');
select ok(not (select prosecdef from pg_proc where oid = 'public.create_quest_beat_with_route(uuid,text,text,double precision,double precision,uuid)'::regprocedure), 'creation uses caller RLS');
select ok(not has_function_privilege('anon', 'public.create_quest_beat_with_route(uuid,text,text,double precision,double precision,uuid)', 'EXECUTE'), 'anonymous creation is revoked');
select ok(has_function_privilege('authenticated', 'public.create_quest_beat_with_route(uuid,text,text,double precision,double precision,uuid)', 'EXECUTE'), 'authenticated DMs can call creation');
-- #795 dropped and recreated this function with a shorter argument list (the
-- route's p_edge_label is gone). `create or replace` with a changed signature
-- creates a silent second overload rather than replacing the original — pin
-- that exactly one survives.
select is(
  (select count(*)::integer from pg_proc where proname = 'create_quest_beat_with_route' and pronamespace = 'public'::regnamespace),
  1, 'exactly one overload of create_quest_beat_with_route exists'
);
...
select lives_ok($$ select public.create_quest_beat_with_route('68600000-0000-4000-8000-000000000020', '  Next scene  ', 'social', 320, 40, '68600000-0000-4000-8000-000000000030') $$, 'DM atomically creates an add-next beat');
...
select is(
  (select count(*)::integer from public.quest_beat_edges e join public.quest_beats b on b.id = e.target_beat_id
    where e.source_beat_id = '68600000-0000-4000-8000-000000000030' and b.title = 'Next scene'),
  1, 'incoming route is created'
);
